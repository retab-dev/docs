...
app.post('/webhook', async (req, res) => {
    const { document, json } = req.body;
    
    if (json) {
        console.log(`📬 Received invoice: ${json.invoice_number} - $${json.total_amount}`);
    }
...
Alternatively, you can manage keys in your [Retab Dashboard](https://www.retab.com/dashboard/settings) under **Settings** → **Security**.
...
app.post('/webhook', async (req, res) => {
    ...
    console.log('Verified webhook:', { document, json });
