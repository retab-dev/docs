...
4. Collects the artifact data, declared handle outputs, routing decision,
   warnings, and duration.
5. Resolves the assertion target from the block artifacts or handles.
...
Each run record includes the saved inputs used for that run, the block artifact
data, handle outputs, assertion result, warnings, duration, and execution
...
Tests are tied to the block inputs, artifact schema, and handle schemas that
existed when the test was created or last updated. If the workflow draft changes,
Retab compares the saved fingerprints and schema dependencies against the
current block.
...
3. Pick the block artifact field or handle you want to protect.
...
tests over one broad assertion so failures point directly to the changed
artifact data or handle.
