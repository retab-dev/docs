...
2. Poll [`GET /v1/jobs/{job_id}`](/api-reference/jobs/retrieve) or
   `client.jobs.retrieve(job_id)` until `status: "completed"`, then read the
   batch result from `response.body`. If the job ends in `failed`,
   `cancelled`, or `expired`, inspect the job's `error` field.
