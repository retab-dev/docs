...
**Consensus Mode:**  
When `n_consensus > 1`, the block runs multiple extractions in parallel and returns:
...
`classifier`, or `conditional`. When the gate predicate fires, that same block
enters `waiting_for_human` and a [review overlay](/workflows/HIL-Reviews) is
created for the block run.
...
The agent's proposal lands on the gated block's [review overlay](/workflows/HIL-Reviews) like any other actor's — an agent-authored edit or decision is the same shape as a human's, distinguished only by `Actor.kind`. Read and act on it through `client.workflows.runs.reviews.*`.
