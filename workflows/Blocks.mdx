...
[review](/workflows/Reviews) is created for the block run.
...
5. The reviewer can approve, modify, or reject through Reviews.
...
**Configuration keys** (on the gated block's review settings):
...
Example review settings with the agent in `auto` mode:
...
The agent's proposal lands on the gated block's [review](/workflows/Reviews) like any other actor's — an agent-authored edit or decision is the same shape as a human's, distinguished only by `Actor.kind`. Read and act on it through `client.workflows.reviews.*`.
