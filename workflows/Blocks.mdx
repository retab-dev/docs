...
<Note>
  ...
  correction, and a human-created correction all use the same `versions_by_id`
  and `decision.version_id` shape. There is no separate agent-review
  configuration on `config.review`; automated reviewers should append versions
  or submit decisions through `client.workflows.reviews.*`.
</Note>
