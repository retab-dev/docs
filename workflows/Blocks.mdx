...
- File (document to edit) — _only when "Use Template" is off_
...
2. Choose a predicate such as `always`, `any_required_field_null`, or a `json_condition` over `likelihoods.<field>`.
...
`confidence_lt`, `any_required_field_null`, and `json_condition` over `likelihoods.<field>`.
...
Split-by-key `for_each` gates additionally support `any_pages_unassigned`,
which pauses the run when the partition left source pages out of every
partition (for example, a garbled page that yielded no key) so a reviewer can
reassign the dropped pages; the step output and review snapshot list them as
`unassigned_pages`.
