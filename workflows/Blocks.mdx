...
5. The reviewer can approve, create a corrected output version, or reject through Reviews.
6. After approval, the selected `versions_by_id[decision.version_id].snapshot` continues through the workflow.
...
<Note>
The review object is actor-neutral: a model output, an agent-created correction,
and a human-created correction all use the same `versions_by_id` and
`decision.version_id` shape. There is no separate agent-review configuration on
`config.review`; automated reviewers should append versions or submit decisions
through `client.workflows.reviews.*`.
</Note>
