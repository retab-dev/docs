...
**How It Works:**
1. Connect to an Extract or Function block (schema is automatically inherited)
2. When the workflow runs, it pauses at the HIL block
...
<Note>
The HIL block automatically inherits the JSON schema from the connected upstream block. It also preserves any computed fields from Function blocks.
</Note>

#### AI Agent reviewer (optional)

An Anthropic managed agent can sit alongside the human flow, ground each flagged field against the source document, and emit a structured proposal (approve / approve‑with‑modifications / reject / escalate). The human always owns the final decision — the agent either pre‑fills it (so a verifier can accept in one click) or, in the higher‑trust modes, applies it directly when its confidence and evidence clear the bar.

**Configuration keys** (on the HIL block):
...
| Mode | If agent approves (high confidence) | If agent approves with corrections | If agent escalates |
|---|---|---|---|
...
| `auto` | Auto‑submitted if `confidence ≥ threshold`, else `escalated` (parked) | Same threshold gate | Always parked (`status=escalated`) |
...
Example HIL block config with the agent in `auto` mode:
...
The agent's proposal lands on the block's [review overlay](/workflows/HIL-Reviews) like any other actor's — an agent-authored edit or decision is the same shape as a human's, distinguished only by `Actor.kind`. Read and act on it through `client.workflows.runs.reviews.*`.
