...
Tests are block-scoped, not full-workflow eval suites. Running an eval replays
...
conditionals, review gate state, and loops are not currently testable
through workflow evals.

## What a Test Stores

A workflow eval definition stores three things:

1. **The block to eval** - the block id and block type.
...
`handle_inputs` from that run into the eval. If the inputs include files, Retab
materializes durable file references for the eval so future runs do not depend
...
You can run one eval from its detail page, run evals for a block, or run all
evals in a workflow from the Evals page.

When a eval runs, Retab:
...
Eval runs are asynchronous. The dashboard streams live progress and stores a
parent run plus one result per saved eval. API clients inspect the parent run by
`run_id`, then read results under `/v1/workflows/evals/results?run_id={run_id}`.

## Results

A parent eval run uses the shared run lifecycle:

| Status        | Meaning                                                         |
| ------------- | --------------------------------------------------------------- |
| **pending**   | The run has been scheduled but has not started yet.             |
| **running**   | One or more eval results are still executing.                   |
...
If a block replay fails before an assertion can run, the assertion is marked as
blocked and the run is recorded as an error.

## Staleness and Schema Drift

Tests are tied to the block inputs and output schema that existed when the eval
...
In the dashboard, the staleness badge tells you whether an eval is:

| Badge          | Meaning                                                                                                |
| -------------- | ------------------------------------------------------------------------------------------------------ |
| **not run**    | The test has not produced a run record yet.                                                            |
| **up to date** | The latest run still matches the current block schema.                                                 |
| **stale**      | The block schema or relevant assertion dependency changed. Run the eval again or update the assertion. |
| **unknown**    | Retab could not determine staleness for this eval.                                                     |

Staleness does not automatically mean the workflow is broken. It means the saved
eval definition should be rechecked against the current draft before you rely on
its latest result.

## Recommended Workflow

1. Run the workflow on a representative document.
2. Open the Evals page and create a workflow eval from that completed run.
