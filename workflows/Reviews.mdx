...
When a workflow reaches a block with `config.review` and the predicate fires, the run pauses and creates a review. A review is addressed by one opaque `id` such as `rev_...`; workflow context like `workflow_run_id`, `block_id`, `step_id`, and `iteration_key` is metadata for filtering and grounding.
...
Append a complete replacement snapshot, then approve the returned `version_id`.
