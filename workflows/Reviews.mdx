...
Review queue items also include `seed_version_id` and `version_count` so you can open the runtime-created seed snapshot without embedding version history in the review object.
...
seed_version = client.workflows.reviews.versions.get(item.seed_version_id)

print(review.workflow_run_id, review.step_id)
