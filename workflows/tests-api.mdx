...
`latest_run_summary.status`, and `WorkflowTestBatchExecutionItem.status`.
...
After [Execute Workflow Tests](/api-reference/workflows/tests/execute),
poll the returned workflow-test `run_id`. Expect transient `queued` /
`running` parent-run states before terminal counts and per-test results are
available.
...
1. `POST /v1/workflows/{wf}/tests/runs` -> returns `{run_id, status: "queued"}` immediately.
2. Poll `GET /v1/workflows/{wf}/tests/runs/{run_id}` until `status` is
   `completed` or `failed`, then fetch results from
   `GET /v1/workflows/{wf}/tests/runs/{run_id}/results`.
3. Workflow-test run results shape:
    - `counts` — one bucket per run-record status (7 fields)
    - `results[]` — one run record per test, with `id` you can pass to [Get Workflow Test Run](/api-reference/workflows/tests/runs/get) for the full snapshot.
...
| Method | Path | Purpose |
|---|---|---|
| `POST` | `/v1/workflows/{wf}/tests` | [Create](/api-reference/workflows/tests/create) |
| `GET` | `/v1/workflows/{wf}/tests` | [List](/api-reference/workflows/tests/list) |
| `GET` | `/v1/workflows/{wf}/tests/{id}` | [Get](/api-reference/workflows/tests/get) |
| `PATCH` | `/v1/workflows/{wf}/tests/{id}` | [Update](/api-reference/workflows/tests/update) |
| `DELETE` | `/v1/workflows/{wf}/tests/{id}` | [Delete](/api-reference/workflows/tests/delete) |
| `POST` | `/v1/workflows/{wf}/tests/runs` | [Execute](/api-reference/workflows/tests/execute) |
| `GET` | `/v1/workflows/{wf}/tests/{id}/runs` | [List Runs](/api-reference/workflows/tests/runs/list) |
| `GET` | `/v1/workflows/{wf}/tests/{id}/runs/{run_id}` | [Get Run](/api-reference/workflows/tests/runs/get) |
