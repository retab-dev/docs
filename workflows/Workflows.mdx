...
while run.lifecycle.kind not in terminal_kinds and run.lifecycle.kind != "waiting_for_human":
    time.sleep(1)
...
elif run.lifecycle.kind == "error":
    raise RuntimeError(run.lifecycle.message)
elif run.lifecycle.kind == "cancelled":
...
func main() {
	...
	for !run.Terminal() && run.Lifecycle.Kind != "waiting_for_human" {
	...
	fmt.Println(run.Lifecycle.Kind)
	if run.Lifecycle.Kind == "waiting_for_human" {
		fmt.Println(run.Lifecycle.WaitingForBlockIDs)
		return
	}
	if !run.Completed() {
		log.Fatalf("workflow finished with lifecycle %s", run.Lifecycle.Kind)
	}
...
for step in client.workflows.runs.steps.list(run.id):
    print(step.block_id, step.lifecycle.kind)
    if step.artifact:
...
for _, step := range steps {
	fmt.Println(step.BlockID, step.Lifecycle["kind"])
...
fmt.Println(step.Lifecycle["kind"])
...
and poll the run until `lifecycle.kind` reaches `completed`, `error`, `cancelled`,
