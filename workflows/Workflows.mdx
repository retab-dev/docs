...
A workflow is fundamentally an asynchronous job. When you start it, Retab creates a workflow run, executes each step on the server, and stores artifact-backed execution data on that run. You can then poll the run until it finishes and inspect the stored step executions.
