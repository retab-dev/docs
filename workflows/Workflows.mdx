...
const result = await client.workflows.specs.apply(yamlDefinition, { publish: false });
...
func main() {
	...
	run, err = client.Workflows.Runs.WaitForCompletion(ctx, run.ID, &retab.WaitForCompletionParams{
	...
	if !run.Completed() {
		log.Fatalf("workflow finished with status %s", run.Lifecycle.Kind)
	}
