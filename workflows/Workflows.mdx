...
Workflow run inputs are keyed by the IDs of your `start-document` and `start_json` blocks. List the workflow's blocks to discover them.
...
document_start_id = next(block.id for block in blocks.data if block.type == "start-document")
...
const documentStartId = blocks.data.find((block) => block.type === "start-document")?.id;
...
blocks = client.workflows.blocks.list(workflow.id)
start_document_block = next(block for block in blocks.data if block.type == "start-document")
...
const blocks = await client.workflows.blocks.list(workflow.id);
const startDocumentBlock = blocks.data.find((block) => block.type === "start-document");
