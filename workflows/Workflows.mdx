...
The Python and Go SDKs expose workflow metadata, graph authoring, run execution, and typed step inspection:
...
if step.Artifact != nil {
	extraction, err := client.Extractions.Get(ctx, step.Artifact.ID)
	if err != nil {
		log.Fatal(err)
	}
	fmt.Println((*extraction)["choices"]) // full consensus, likelihoods, schema
}
```
