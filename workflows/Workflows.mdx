...
const exported = await client.workflows.specs.get(result.workflowId);
```

```php PHP
$validation = $client->workflowSpecs()->validate($yamlDefinition);
$plan = $client->workflowSpecs()->plan($yamlDefinition);
$result = $client->workflowSpecs()->apply($yamlDefinition);
$exported = $client->workflowSpecs()->get($result->workflowId);
```
...
exported = client.workflow_specs.get(workflow_id: result.workflow_id)
```
...
var validation = await client.WorkflowSpecs.ValidateAsync(
    client.ApiKey,
    new WorkflowSpecsValidateOptions { YamlDefinition = yamlDefinition }
);
var plan = await client.WorkflowSpecs.PlanAsync(
    client.ApiKey,
    new WorkflowSpecsPlanOptions { YamlDefinition = yamlDefinition }
);
var result = await client.WorkflowSpecs.ApplyAsync(
    client.ApiKey,
    new WorkflowSpecsApplyOptions { YamlDefinition = yamlDefinition }
);
var exported = await client.WorkflowSpecs.GetAsync(result.WorkflowId, client.ApiKey);
```
...
func main() {
	...
	validation, err := client.Workflows.Spec.Validate(context.Background(), yamlDefinition)
	if err != nil {
		log.Fatal(err)
	}
	plan, err := client.Workflows.Spec.Plan(context.Background(), yamlDefinition)
	if err != nil {
		log.Fatal(err)
	}
	result, err := client.Workflows.Spec.Apply(context.Background(), yamlDefinition)
	if err != nil {
		log.Fatal(err)
	}
	workflowID, _ := (*result)["workflow_id"].(string)
	exported, err := client.Workflows.Spec.Get(context.Background(), workflowID)
	if err != nil {
		log.Fatal(err)
	}

	fmt.Println(validation, plan, result, exported)
}
```

```rust Rust
use retab::{models::DeclarativeWorkflowRequest, resources::workflow_specs, Retab};

let client = Retab::new(std::env::var("RETAB_API_KEY")?);
let validation = client
    .workflow_specs()
    .validate(workflow_specs::ValidateParams::new(DeclarativeWorkflowRequest::new(yaml_definition.clone())))
    .await?;
let plan = client
    .workflow_specs()
    .plan(workflow_specs::PlanParams::new(DeclarativeWorkflowRequest::new(yaml_definition.clone())))
    .await?;
let result = client
    .workflow_specs()
    .apply(workflow_specs::ApplyParams::new(DeclarativeWorkflowRequest::new(yaml_definition)))
    .await?;
let exported = client.workflow_specs().get(&result.workflow_id).await?;

println!("{validation:#?} {plan:#?} {exported:#?}");
```
...
blocks = client.workflow_blocks.list(workflow_id: 'wf_abc123')
...
$blocks = $client->workflowBlocks()->list('wf_abc123');
...
var blocks = await client.WorkflowBlocks.ListAsync(
    client.ApiKey,
...
blocks = client.workflow_blocks.list(workflow_id: workflow.id)
document_start_id = blocks.data.find { |block| block.type == 'start_document' }&.id
json_start_id = blocks.data.find { |block| block.type == 'start_json' }&.id
...
run = client.workflow_runs.create(
  workflow_id: workflow.id,
  documents: {
    document_start_id => { filename: 'invoice.pdf', url: 'path/to/invoice.pdf' },
  },
  json_inputs: {
    json_start_id => { customer_id: 'cust_123', priority: 'high' },
  },
...
until terminal_statuses.include?(run.lifecycle.status) || run.lifecycle.status == 'awaiting_review'
  sleep 1
  run = client.workflow_runs.get(run_id: run.id)
end
...
when 'cancelled'
  ...
  client.workflow_steps.list(run_id: run.id).data.each do |step_summary|
    step = client.workflow_steps.get(step_id: step_summary.step_id, run_id: run.id)
...
$workflow = $client->workflows()->get('wf_abc123');
$blocks = $client->workflowBlocks()->list($workflow->id);
...
$run = $client->workflowRuns()->create(
    workflowId: $workflow->id,
    documents: [
        $documentStartId => ['url' => 'path/to/invoice.pdf'],
    ],
    jsonInputs: [
        $jsonStartId => ['customer_id' => 'cust_123', 'priority' => 'high'],
    ],
...
while (!in_array($run->lifecycle->status, $terminalStatuses, true)
    ...
    sleep(1);
    $run = $client->workflowRuns()->get($run->id);
}
...
if ($run->lifecycle->status === 'awaiting_review') {
    ...
    foreach ($client->workflowSteps()->list(runId: $run->id)->data as $stepSummary) {
        $step = $client->workflowSteps()->get($stepSummary->stepId, runId: $run->id);
...
```rust Rust
use retab::{models::CreateFreshWorkflowRunRequest, resources::workflow_runs, Retab};
use std::time::{Duration, Instant};

let client = Retab::new(std::env::var("RETAB_API_KEY")?);
let mut run = client
    .workflow_runs()
    .create(workflow_runs::CreateParams::new(
        CreateFreshWorkflowRunRequest::new("wf_abc123").into(),
    ))
    .await?;

let deadline = Instant::now() + Duration::from_secs(600);
while !matches!(run.lifecycle.status.as_deref(), Some("completed" | "error" | "cancelled" | "awaiting_review")) {
    if Instant::now() > deadline {
        return Err("timed out waiting for workflow run".into());
    }
    tokio::time::sleep(Duration::from_secs(1)).await;
    run = client.workflow_runs().get(&run.id).await?;
}
...
var client = new Retab("YOUR_API_KEY");
var run = await client.WorkflowRuns.CreateAsync(
    client.ApiKey,
    new WorkflowRunsCreateOptions { WorkflowId = "wf_abc123" }
);

var deadline = DateTimeOffset.UtcNow.AddMinutes(10);
while (run.Lifecycle.Status is not ("completed" or "error" or "cancelled" or "awaiting_review"))
{
    ...
    run = await client.WorkflowRuns.GetAsync(run.Id, client.ApiKey);
}

Console.WriteLine(run.Lifecycle.Status);
```
...
```ruby Ruby
# Step roster:
client.workflow_steps.list(run_id: run.id).data.each do |step|
...
# Full execution record for one step:
step = client.workflow_steps.get(step_id: 'extract-step-id', run_id: run.id)
...
<?php
// Step roster
foreach ($client->workflowSteps()->list(runId: $run->id)->data as $step) {
...
// Full execution record for one step:
$step = $client->workflowSteps()->get('extract-block-id', runId: $run->id);
...
```rust Rust
use retab::{resources::workflow_steps::ListParams, Retab};
...
let step = client.workflow_steps().get("extract-step-id").await?;
...
var client = new Retab("YOUR_API_KEY");
var steps = await client.WorkflowSteps.ListAsync(
    client.ApiKey,
...
{
    Console.WriteLine($"{summary.BlockId} {summary.Lifecycle.Status}");
...
var step = await client.WorkflowSteps.GetAsync("extract-step-id", client.ApiKey);
...
```ruby Ruby
step = client.workflow_steps.get(step_id: 'conditional-step-id', run_id: run.id)
if step.artifact
  artifact = client.workflow_artifacts.get(artifact_id: step.artifact.id)
...
```php PHP
$step = $client->workflowSteps()->get('conditional-block-id', runId: $run->id);
if ($step->artifact !== null) {
    $artifact = $client->workflowArtifacts()->get($step->artifact->id);
...
func main() {
	...
	step, err := client.Workflows.Steps.Get(context.Background(), "conditional-step-id")
	if err != nil {
		log.Fatal(err)
	}
	if step.Artifact != nil {
...
let client = Retab::new(std::env::var("RETAB_API_KEY")?);
let step = client.workflow_steps().get("conditional-step-id").await?;
if let Some(artifact_ref) = step.artifact {
    let artifact = client.workflow_artifacts().get(&artifact_ref.id).await?;
    println!("{artifact:#?}");
}
```
...
var client = new Retab("YOUR_API_KEY");
var step = await client.WorkflowSteps.GetAsync("conditional-step-id", client.ApiKey);
if (step.Artifact is not null)
{
    var artifact = await client.WorkflowArtifacts.GetAsync(step.Artifact.Id, client.ApiKey);
...
```ruby Ruby
condition_records = client.workflow_artifacts.list(
  run_id: run.id,
  operation: 'conditional_evaluation',
)
```
...
$conditionRecords = $client->workflowArtifacts()->list(
    runId: $run->id,
...
func main() {
	...
	records, err := client.Workflows.Artifacts.List(context.Background(), retab.ListWorkflowArtifactsParams{
		RunID:     run.ID,
		Operation: "conditional_evaluation",
	})
	if err != nil {
...
var client = new Retab("YOUR_API_KEY");
var conditionRecords = await client.WorkflowArtifacts.ListAsync(
    client.ApiKey,
...
extract_block = client.workflow_blocks.create(
  workflow_id: workflow.id,
  id: 'extract-invoice',
  type: 'extract',
  label: 'Extract Invoice',
...
$extractBlock = $client->workflowBlocks()->create(
    workflowId: $workflow->id,
...
var start = blocks.Items.First(block => block.Type == WorkflowBlockType.StartDocument);
var extract = await client.WorkflowBlocks.CreateAsync(
    client.ApiKey,
...
await client.WorkflowEdges.CreateAsync(
    client.ApiKey,
...
await client.Workflows.PublishAsync(workflow.Id, client.ApiKey, new WorkflowsPublishOptions());
```
