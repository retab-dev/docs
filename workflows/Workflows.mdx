...
for end_node_id, output in (run.output or {}).items():
    print(end_node_id, output.data, output.document)
```
...
Use `client.workflows.runs.steps.list(run.id)` when you want the persisted step documents for every node, or `client.workflows.runs.steps.list(run.id, node_ids=[...])` when you only need a subset.
