...
const result = await client.workflows.spec.apply(yamlDefinition);
const exported = await client.workflows.spec.get(result.workflowId);
```

```php PHP
$validation = $client->workflows()->spec()->validate($yamlDefinition);
$plan = $client->workflows()->spec()->plan($yamlDefinition);
$result = $client->workflows()->spec()->apply($yamlDefinition);
$exported = $client->workflows()->spec()->get($result->workflowId);
```
...
result = client.workflows.spec.apply(yaml_definition: yaml_definition)
exported = client.workflows.spec.get(workflow_id: result.workflow_id)
```
...
var result = await client.Workflows.Spec.ApplyAsync(
    new WorkflowSpecApplyOptions { YamlDefinition = yamlDefinition }
);
var exported = await client.Workflows.Spec.GetAsync(result.WorkflowId);
```
...
`
	...
	result, err := client.Workflows.Spec.Apply(context.Background(), &retab.WorkflowSpecApplyParams{YamlDefinition: yamlDefinition})
	if err != nil {
		log.Fatal(err)
	}
	exported, err := client.Workflows.Spec.Get(context.Background(), result.WorkflowID)
	if err != nil {
		log.Fatal(err)
	}
...
let result = client
    .workflows().spec()
    .apply(workflow_spec::ApplyParams::new(DeclarativeWorkflowRequest::new(yaml_definition)))
    .await?;
let exported = client.workflows().spec().get(&result.workflow_id).await?;
...
func main() {
	...
	for {
		status := ""
		if run.Lifecycle != nil && run.Lifecycle.Status != nil {
...
while !matches!(
    run.lifecycle,
    Some(retab::models::WorkflowRunLifecycleOneOf::CompletedTerminal(_))
        | Some(retab::models::WorkflowRunLifecycleOneOf::ErrorTerminal(_))
        | Some(retab::models::WorkflowRunLifecycleOneOf::CancelledTerminal(_))
        | Some(retab::models::WorkflowRunLifecycleOneOf::AwaitingReviewRun(_))
) {
...
public final class Example {
  public static void main(String[] args) throws Exception {
    ...
    var result = client.workflows().steps().list("run_abc123", "block_abc123", null, "step_abc123", null, null, null, null, 10L);
    System.out.println(result);
  }
}
```
