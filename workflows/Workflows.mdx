...
- [Validate Workflow Spec](/api-reference/workflows/specs/validate)
- [Plan Workflow Spec](/api-reference/workflows/specs/plan)
- [Apply Workflow Spec](/api-reference/workflows/specs/apply)
- [Export Workflow Spec](/api-reference/workflows/specs/export)
...
- `client.workflows.*` / `client.Workflows.*` for `list()`, `get()`, `create()`, `publish()`, `duplicate()`, and `get_entities()`
...
- `client.workflows.runs.*` / `client.Workflows.Runs.*` and `client.workflows.runs.steps.*` / `client.Workflows.Runs.Steps.*` for running flows and reading results
...
const workflow = await client.workflows.getEntities("wf_abc123");

const documentStartId = workflow.blocks.find((block) => block.type === "start")?.id;
const jsonStartId = workflow.blocks.find((block) => block.type === "start_json")?.id;
...
func main() {
	...
	workflow, err := client.Workflows.GetEntities(ctx, "wf_abc123")
	if err != nil {
		log.Fatal(err)
	}
...
while run.lifecycle.status not in terminal_statuses and run.lifecycle.status != "waiting_for_human":
    time.sleep(1)
...
while (!terminalStatuses.has(run.lifecycle.status) && run.lifecycle.status !== "waiting_for_human") {
...
func main() {
	...
	for !run.Terminal() && run.Lifecycle.Status != "waiting_for_human" {
	...
	steps, err := client.Workflows.Runs.Steps.List(ctx, run.ID)
	if err != nil {
		log.Fatal(err)
	}
	for _, stepSummary := range steps {
		step, err := client.Workflows.Runs.Steps.Get(ctx, run.ID, stepSummary.BlockID)
		if err != nil {
			log.Fatal(err)
		}
...
// Full execution record for one step:
const step = await client.workflows.runs.steps.get(run.id, "extract-block-id");
...
// Full execution record for one step:
step, err := client.Workflows.Runs.Steps.Get(ctx, run.ID, "extract-block-id")
if err != nil {
	log.Fatal(err)
}
...
```json
{ "operation": "hil_evaluation", "id": "heval_abc123" }
```
...
<CodeGroup>
```python Python
step = client.workflows.runs.steps.get(run.id, "hil-block-id")
if step.artifact:
...
```javascript JavaScript
const step = await client.workflows.runs.steps.get(run.id, "function-block-id");
if (step.artifact) {
...
<CodeGroup>
```python Python
hil_records = client.workflows.artifacts.list(
    run.id,
...
```javascript JavaScript
const hilRecords = await client.workflows.artifacts.list({
  runId: run.id,
  operation: "hil_evaluation",
});
```
...
| `hil_evaluation` | `hil` | condition evaluations plus review state such as `requires_human_review`, `review_decision`, `review_notes` |
...
const entities = await client.workflows.getEntities(workflow.id);
const startBlock = entities.blocks.find((block) => block.type === "start");

if (!startBlock) {
  throw new Error("Workflow is missing a start block");
}
...
entities, err := client.Workflows.GetEntities(ctx, workflow.ID)
if err != nil {
	log.Fatal(err)
}

var startBlock retab.WorkflowBlock
...
3. Poll the workflow run until it finishes or waits for human review
