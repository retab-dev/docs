...
Start with `steps.list(run.id)` — it returns every persisted step in a single HTTP call. Avoid looping over `run.steps` and calling `steps.get()` per block; that's N+1.

Step payloads are normalized into `HandlePayload` objects. For JSON-producing blocks, read the default `output-json-0` handle.
...
for step in client.workflows.runs.steps.list(run.id):
    ...
    if step.execution.outputs:
        ...
        json_output = payload.data if payload and payload.type == "json" else None
    print(step.block_id, step.status, step.error, json_output)
    for artifact in step.execution.artifacts:
        print(artifact.operation, artifact.id)

# Single step:
step = client.workflows.runs.steps.get(run.id, "extract-block-id")
payload = step.execution.outputs.get("output-json-0")
print(step.status, payload.data if payload and payload.type == "json" else None)
```
...
Inference blocks persist resources; `step.execution.artifacts` contains `{operation, id}` pointers you use to fetch the full typed results:
