...
const created = await client.workflows.spec.apply(yamlDefinition);
const updated = await client.workflows.spec.apply_to_workflow("wf_abc123", yamlDefinition);
...
$plan = $client->workflows()->spec()->plan($yamlDefinition);
$created = $client->workflows()->spec()->apply($yamlDefinition);
$updated = $client->workflows()->spec()->applyToWorkflow('wf_abc123', $yamlDefinition);
...
created = client.workflows.spec.apply(yaml_definition: yaml_definition)
updated = client.workflows.spec.apply_to_workflow(workflow_id: 'wf_abc123', yaml_definition: yaml_definition)
...
var plan = await client.Workflows.Spec.PlanAsync(
    new WorkflowSpecPlanOptions { YamlDefinition = yamlDefinition }
);
var created = await client.Workflows.Spec.ApplyAsync(
    new WorkflowSpecApplyOptions { YamlDefinition = yamlDefinition }
);
var updated = await client.Workflows.Spec.ApplyToWorkflowAsync(
    "wf_abc123",
    new WorkflowSpecApplyToWorkflowOptions { YamlDefinition = yamlDefinition }
);
...
`
	...
	plan, err := client.Workflows.Spec.Plan(context.Background(), &retab.WorkflowSpecPlanParams{YamlDefinition: yamlDefinition})
	if err != nil {
		log.Fatal(err)
	}
	created, err := client.Workflows.Spec.Apply(context.Background(), &retab.WorkflowSpecApplyParams{YamlDefinition: yamlDefinition})
	if err != nil {
		log.Fatal(err)
	}
	updated, err := client.Workflows.Spec.ApplyToWorkflow(context.Background(), "wf_abc123", &retab.WorkflowSpecApplyToWorkflowParams{YamlDefinition: yamlDefinition})
	if err != nil {
		log.Fatal(err)
	}
...
```rust Rust
use retab::{models::DeclarativeWorkflowRequest, resources::workflow_spec, Retab};
...
let plan = client
    .workflows().spec()
    .plan(workflow_spec::PlanParams::new(DeclarativeWorkflowRequest::new(yaml_definition.clone())))
    .await?;
let created = client
    .workflows().spec()
    .apply(workflow_spec::ApplyParams::new(DeclarativeWorkflowRequest::new(yaml_definition.clone())))
    .await?;
let updated = client
    .workflows().spec()
    .apply_to_workflow(
        "wf_abc123",
        workflow_spec::ApplyToWorkflowParams::new(DeclarativeWorkflowRequest::new(yaml_definition.clone())),
    )
    .await?;
...
`apply()` to create a new workflow from YAML, `apply_to_workflow()` to modify an
existing workflow draft, and `get()` to get canonical YAML from an existing
workflow.
...
- [Apply Workflow Spec to Existing Workflow](/api-reference/workflows/spec/apply-to)
...
public final class Example {
  public static void main(String[] args) throws Exception {
    ...
    var result = client.workflows().create("Invoice Processing", "Extract invoice fields");
    System.out.println(result);
  }
}
```
