...
Workflow run inputs are keyed by the IDs of your `start-document` and `start_json` blocks. `get_entities()` is the easiest way to discover them.
...
const documentStartId = workflow.blocks.find((block) => block.type === "start-document")?.id;
...
- `documents` for Document (`start-document`) blocks
...
<CodeGroup>
```python Python
step = client.workflows.runs.steps.get(run.id, "conditional-block-id")
if step.artifact:
...
start_document_block = entities.start_document_blocks[0]
...
const startDocumentBlock = entities.blocks.find((block) => block.type === "start-document");

if (!startDocumentBlock) {
  throw new Error("Workflow is missing a start-document block");
}
...
<AccordionGroup>
  ...
  <Accordion title="Validate with Human-in-the-Loop">
    For critical data, add a HIL gate to the extraction block. This ensures a human reviews low-likelihood results before they proceed.
  </Accordion>
...
3. The extract block's **HIL gate** flags low-likelihood extractions for human review
