...
description: Reusable per-block checks with discriminated target/source shapes, async eval runs, and per-eval result records
---

Workflow evals let you freeze the inputs to a single workflow block and assert
...
[Workflow Evals](/workflows/evals).

## What's in an eval

A `WorkflowEval` is a Pydantic model with three meaningful sections:
...
### `target` — what the eval runs against

A discriminated union by `type`:

| `type`  | Fields     | Meaning                                              |
| ------- | ---------- | ---------------------------------------------------- |
| `block` | `block_id` | Run the eval against a single block in the workflow. |
...
| `type`     | Fields                                        | Meaning                                                                                                                                                                       |
| ---------- | --------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `manual`   | `handle_inputs: { [handle_id]: HandleInput }` | Hand-written inputs. Use for synthetic eval cases.                                                                                                                            |
...
When you create an eval from `run_step`, Retab snapshots the inputs at create
time — subsequent edits to the source workflow run don't affect the eval.
File handles in the snapshot are materialized as durable Retab file refs so
the eval still runs months later even if the original upload session is gone.

### `assertion` — required, one per eval
...
Workflow evals intentionally normalize to **one assertion per eval**. Multiple
small evals beat one broad assertion: when an assertion fails, the failure
...
inside each handle type, lives at [Workflow Evals](/workflows/evals).

## Two distinct status enums — don't confuse them

An eval surfaces TWO status fields. Most surprises with the API trace back to
mixing them up.
...
The status of a workflow-eval run — aggregates the assertion result with execution-side
state. This is what appears on `WorkflowEvalResult.status`,
`latest_run_summary.status`, and per-eval result rows returned from
`/v1/workflows/evals/results?run_id={run_id}`.
...
| `blocked`   | Execution finished but the assertion was blocked (see above). Counted distinctly from `failed` because the user typically needs to fix the eval definition or the block's outputs, not the assertion expectation. |
...
After [Create Workflow Eval Run](/api-reference/workflows/evals/runs/create),
poll the returned workflow-eval run `id`. Expect transient `pending` /
`running` parent-run lifecycle states before terminal counts and per-eval
results are available.

Workflow-eval execution uses the returned run `id` for polling, cancellation,
and result inspection.

## Run records

A `WorkflowEvalResult` is the immutable snapshot of one execution. The
...
| `block_config_fingerprint`   | The single block's resolved config | Same as above, scoped to just the evaled block — gives finer-grained staleness signals.                                |
...
When the workflow draft changes (schema edited, block config tweaked), evals
...
| `partial`      | Some assertion paths still resolve, others don't. The eval will likely produce a `blocked` result. |
...
Workflow-eval execution is asynchronous. The flow:

1. `POST /v1/workflows/evals/runs` with `workflow_id` in the request body
   returns a run object immediately.
2. Poll `GET /v1/workflows/evals/runs/{run_id}` until `lifecycle.status` is
   `completed`, `error`, or `cancelled`, then fetch results from
   `GET /v1/workflows/evals/results?run_id={run_id}`.
3. Workflow-eval run results shape:
   - `counts` — one bucket per run-record status (7 fields)
   - `data[]` — one result per eval, keyed by `eval_id` within the parent run.

For dashboard integrations, poll the parent run status and refresh eval-run
...
Every endpoint above is also exposed as an MCP tool (`workflows_evals_create`,
`workflows_evals_list`, `workflows_evals_get`, `workflows_evals_update`,
`workflows_evals_delete`, `workflows_evals_runs_create`,
`workflows_evals_runs_get`, `workflows_evals_results_list`,
`workflows_evals_results_get`). The tool
