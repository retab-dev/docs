...
```javascript Node
const currentRun = await client.workflows.experiments.runs.get({
  runId: run.id,
});
...
for (const result of results.data) {
  console.log(result.document_id, result.lifecycle.status);
}
...
`/v1/workflows/experiments/runs/{run_id}/metrics`.
...
```javascript Node
await client.workflows.experiments.update("wf_abc123", experiment.id, {
  nConsensus: 7,
});

await client.workflows.experiments.delete("wf_abc123", experiment.id);
```
