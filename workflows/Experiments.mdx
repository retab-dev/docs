...
var experiment = await client.WorkflowExperiments.CreateAsync(
    client.ApiKey,
...
request.n_consensus = Some(CreateExperimentRequestNConsensus::Value5);
...
var run = await client.ExperimentRuns.CreateAsync(
    client.ApiKey,
...
Console.WriteLine($"{run.Id} {run.Lifecycle.Status}");
```
...
let client = Retab::new(std::env::var("RETAB_API_KEY")?);
let mut request = CreateExperimentRunRequest::new(&experiment.id);
...
var currentRun = await client.ExperimentRuns.GetAsync(run.Id, client.ApiKey);
Console.WriteLine($"{currentRun.Id} {currentRun.Lifecycle.Status}");

var results = await client.ExperimentRunResults.ListAsync(
    client.ApiKey,
    new ExperimentRunResultsListOptions { RunId = run.Id }
);
await foreach (var item in results)
{
    Console.WriteLine($"{item.DocumentId} {item.Lifecycle.Status}");
}

var result = await client.ExperimentRunResults.GetAsync("expdoc_xyz", client.ApiKey);
Console.WriteLine(result.Artifact);
```

```rust Rust
use retab::{resources::{experiment_run_results, experiment_runs}, Retab};

let client = Retab::new(std::env::var("RETAB_API_KEY")?);
let current_run = client.experiment_runs().get(&run.id).await?;
println!("{} {:?}", current_run.id, current_run.lifecycle);

let results = client
    .experiment_run_results()
    .list(experiment_run_results::ListParams::new(&run.id))
    .await?;
for result in results.data {
    println!("{} {:?}", result.document_id, result.lifecycle);
}

let result = client.experiment_run_results().get("expdoc_xyz").await?;
...
var summary = await client.ExperimentRunMetrics.GetAsync(
    client.ApiKey,
...
// A weak field surfaces in summary.aggregate.likelihoods - drill in
var targetView = await client.ExperimentRunMetrics.GetAsync(
    client.ApiKey,
    new ExperimentRunMetricsGetOptions
    {
        RunId = run.Id,
...
// To see what each voter said for one document/target cell
var votes = await client.ExperimentRunMetrics.GetAsync(
    client.ApiKey,
    new ExperimentRunMetricsGetOptions
    {
        RunId = run.Id,
...
let summary = client
    ...
    .get(experiment_run_metrics::GetParams::new(&run.id))
    .await?;

let mut target_params = experiment_run_metrics::GetParams::new(&run.id);
target_params.view = Some(ExperimentRunMetricsView::ByTarget);
target_params.target_path = Some("line_items.*.unit_price".to_string());
let target_view = client.experiment_run_metrics().get(target_params).await?;

let mut votes_params = experiment_run_metrics::GetParams::new(&run.id);
...
let votes = client.experiment_run_metrics().get(votes_params).await?;
...
func main() {
	...
	nConsensus := 7
	updated, err := client.Workflows.Experiments.Update(ctx, "exp_abc123", retab.UpdateExperimentRequest{
...
// Change n_consensus — invalidates existing metrics
await client.WorkflowExperiments.UpdateAsync(
    experiment.Id,
    client.ApiKey,
...
await client.WorkflowExperiments.DeleteAsync(experiment.Id, client.ApiKey);
```
...
request.n_consensus = Some(UpdateExperimentRequestNConsensus::Value7);
client
    .workflow_experiments()
    .update(
        &experiment.id,
    ...
    .await?;
client.workflow_experiments().delete(&experiment.id).await?;
```
