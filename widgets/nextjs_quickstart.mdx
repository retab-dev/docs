...
<Tip>
  Get your API key and project ID from the [Retab
  dashboard](https://retab.com/dashboard/settings).
</Tip>
  ...
  try {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_RETAB_BASE_URL}/v1/auth/session`,
      {
        method: "POST",
        headers: {
          "Authorization": `Bearer ${retabApiKey}`,
  ...
  return (
    <RetabProvider
      ...
      authConfig={{
        ...
        baseUrl:
          process.env.NEXT_PUBLIC_RETAB_BASE_URL || "https://api.retab.com",
      }}
    >
      {children}
    </RetabProvider>
...
<Note>
  The root layout remains a Server Component (no "use client"), which allows you
  to use `metadata` exports and other server-only features. The `Providers`
  component handles the client-side context.
</Note>
...
const widgets: Widget[] = [
  ...
  },
  {
    ...
    description:
      "Complete review interface with list, file preview, and data editor",
    category: "composite",
  },
  {
    id: "extraction",
    ...
    description:
      "Side-by-side file preview and data display with resizable panels",
    category: "composite",
  },
  {
    id: "uploads",
...
function WidgetRenderer({ widgetId }: { widgetId: WidgetId }) {
  ...
  switch (widgetId) {
    ...
    case "file":
      return <FileComponent extraction={firstExtraction} fieldPath={null} />;
