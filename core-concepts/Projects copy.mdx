...
3. **Run iterations** with different model settings (GPT-5.2 vs GPT-5.2-mini, consensus, etc.)
...
# Get performance metrics for one document
document_id = project.documents[0].id

for iteration in project.iterations:
    distances = client.projects.iterations.compute_distances(
        project_id=project.id,
