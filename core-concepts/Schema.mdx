...
func main() {
	...
	schemaObj, err := client.Schemas.Generate(ctx, retab.GenerateSchemaRequest{
		Model:     "gpt-5.4",
	...
	fmt.Println((*schemaObj)["json_schema"])
}
```
```json Response
{
...
You can easily identify the fields that require a source by the `source___[attributename]` naming pattern.
