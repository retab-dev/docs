...
If all the LLMs outputs are the same, we can be confident that the output is correct. 

In our platform, we leverage consensus to detect the ambiguity in the queries of the user. If the user has an ambiguous query, the LLMs will have to interpret the intent of the user, and this interpretation can vary across LLM runs. 
