...
- **Aggregation functions**: `SUM`, `AVERAGE`, `MIN`, `MAX`, `COUNT`, `COUNTIF`
...
| COUNT    | `COUNT(array)`               | Count all items       | `COUNT(tasks.*)`                 |
| COUNTIF  | `COUNTIF(array, value)`      | Count occurrences     | `COUNTIF(tasks.*.status, "done")`|
...
```text
COUNTIF(tasks.*.status, "completed")
```
