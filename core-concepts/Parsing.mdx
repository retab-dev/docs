...
```mermaid
flowchart LR
    A[Document] --> B[reclient.documents.parse]
    
    subgraph Retab Parse API
    B --> B1[OCR & Layout] --> C[Structured Text]
    B --> B2[Table Extraction] --> C
    B --> B3[Text Cleaning] --> C
    B --> B4[Page Separation] --> C
    end
...
### 1. Basic Document Text Extraction

Extract clean text from any document type for basic processing or display.

<CodeGroup>
```python Basic Text Extraction
...
print(f"Extracted {len(full_text)} characters from {result.usage.page_count} pages")

# Process individual pages
for i, page_content in enumerate(result.pages, 1):
    print(f"Page {i}: {page_content[:100]}...")
```
</CodeGroup>
