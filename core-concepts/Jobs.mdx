...
if job.status == "completed":
    job = client.jobs.retrieve(job.id, include_response=True)
...
```javascript JavaScript
if (job.status === "completed") {
  job = await client.jobs.retrieve(job.id, { include_response: true });
...
if (job.status === "completed") {
    job = await client.jobs.retrieve(job.id, { include_response: true });
    const extracted = job.response.body.output;
...
if (job.status === "completed") {
    job = await client.jobs.retrieve(job.id, { include_response: true });
    const result = job.response.body.output;
