...
status, _ := (*job)["status"].(string)
if status == "completed" {
    job, err = client.Jobs.RetrieveFull(ctx, jobID)
    if err != nil {
        log.Fatal(err)
    }
...
if jobsResponse.HasMore {
    nextPage, err := client.Jobs.List(ctx, &retab.ListJobsParams{After: jobsResponse.NextCursor})
    if err != nil {
        log.Fatal(err)
    }
    _ = nextPage
...
| `/v1/edits/templates/fill` | Fill a saved template with data |
