...
var client = new Retab(Environment.GetEnvironmentVariable("RETAB_API_KEY")!);
...
while (job.Status is null || !terminalStatuses.Contains(job.Status.Value))
{
    ...
    job = await client.Jobs.GetAsync(job.Id, client.ApiKey);
...
if status == "completed" {
    ...
    if errInfo, ok := (*job)["error"].(map[string]any); ok {
        fmt.Printf("Error: %v\n", errInfo["message"])
    }
}
```
...
var cancelledJob = await client.Jobs.CancelAsync(job.Id, client.ApiKey);
...
foreach (var job in jobsResponse.Items)
{
...
func main() {
	...
	if status == "completed" {
		...
		errInfo, _ := (*job)["error"].(map[string]any)
...
```go Go
job, err := client.Jobs.WaitForCompletion(ctx, jobID, nil)
if err != nil {
    log.Fatal(err)
}
...
if status, _ := (*job)["status"].(string); status == "failed" {
    if errInfo, ok := (*job)["error"].(map[string]any); ok {
        ...
        if details, ok := errInfo["details"]; ok && details != nil {
