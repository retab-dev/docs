...
resource such as an extraction, classification, split, partition, or edit. The
initial response includes an `id` and a lifecycle `status`; poll the resource
until it reaches a terminal state.

<Note>
  Background mode is available on primitive create endpoints. It replaces the
  older generic Jobs API for most client workflows.
</Note>
