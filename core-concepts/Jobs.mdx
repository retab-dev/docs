...
Use `GET /v1/jobs?include_request=true&include_response=true` if you need full payloads in list results. For most workflows, list for discovery and call retrieve on specific job IDs for full details.
...
Note: in list responses (`GET /v1/jobs`) the `request` and `response` fields are `null` by default unless `include_request/include_response` query parameters are enabled.
