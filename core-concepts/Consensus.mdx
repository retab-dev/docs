...
# How to build a great schema with consensus
...
flowchart LR
    ...
    Test --> Inspect[Review Agreement Metrics]
...
class CalendarEvent(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra = {
            "X-SystemPrompt": "Extract the calendar event."
        }
    )
```
...
### Step 3 — Rank Fields by Agreement

| Field          | Agreement (4 samples) | Recommended Action             |
| -------------- | --------------------- | ------------------------------ |
| `name`         | 4/4                   | Retain current configuration   |
| `date`         | 2/4                   | Clarify format or tighten type |
| `address` | 1/4                   | Redesign field data structure       |

A working threshold of **≥ 3/4** is suitable for production deployment.
...
| Good data, wrong format | Instruction unclear | Be more specific in prompt |
| Empty vs filled arrays | Inclusion criteria unclear | Define when to include items |
| Varying data across requests | Prompt ambiguity | Clarify prompt |
