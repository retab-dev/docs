...
As LLMs are non-deterministic, the same prompt can yield different outputs. 
Consensus consists of asking several language‑model instances the **same** question in parallel, then merging their structured outputs into a single, consolidated result.

This procedure allows us to compare the different outputs with each other, and offers a principled way to measure the uncertainty of a given output:
If all the LLMs outputs are the same, we can be confident that the output is correct. Otherwise, measu

In retab, consensus is used to detect the ambiguity in the queries of the user. If the user has an ambiguous query, the LLMs will have to interpret the intent of the user, and this interpretation can vary across LLM runs. 


You can used consensus using retab SDK, but also with our dedicated [k-llms library](https://retab.com/k-llms)
