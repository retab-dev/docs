...
Unlike multi-section routing workflows, `classify` determines the single most appropriate category for the entire document and returns the classification with reasoning.
...
A ClassifyResponse object containing the classification result, consensus metadata, and usage.
  <Expandable title="properties">
    <ResponseField name="classification" type="ClassifyDecision">
      The classification result containing:
      - `category`: The category name that the document belongs to
      - `reasoning`: Step-by-step explanation of why this classification was chosen
    </ResponseField>
    <ResponseField name="consensus" type="ClassifyConsensus">
      Consensus metadata containing:
      - `choices`: Alternative classification vote outputs
      - `likelihood`: Consensus likelihood for the winning classification
    </ResponseField>
    <ResponseField name="usage" type="RetabUsage">
      Usage information for the classification request.
    </ResponseField>
  </Expandable>
</ResponseField>
...
print(f"Document classified as: {result.classification.category}")
print(f"Reasoning: {result.classification.reasoning}")

# Route to appropriate extraction pipeline
if result.classification.category == "invoice":
...
elif result.classification.category == "contract":
...
console.log(`Document classified as: ${result.classification.category}`);
...
} else if (result.classification.category === "contract") {
    // Use contract extraction schema
...
for attachment in email_attachments:
    ...
    elif result.classification.category == "quote":
        # Medium priority - queue for review
        queue_for_review(attachment)
    elif result.classification.category == "marketing":
        # Low priority - skip or archive
        archive_document(attachment)
    
    print(f"{attachment.name}: {result.classification.category}")
    print(f"  Reason: {result.classification.reasoning[:100]}...")
```
...
for (const attachment of emailAttachments) {
    ...
    } else if (result.classification.category === "quote") {
        // Medium priority - queue for review
        await queueForReview(attachment);
    } else if (result.classification.category === "marketing") {
        // Low priority - skip or archive
        await archiveDocument(attachment);
    }
    
    console.log(`${attachment.name}: ${result.classification.category}`);
    console.log(`  Reason: ${result.classification.reasoning.slice(0, 100)}...`);
}
```
</CodeGroup>
...
| **Output** | Single category | Multiple section-level page assignments |
| **Use Case** | Document routing, filtering | Batch separation, section extraction |
| **Input** | Any document | Typically multi-page documents |
| **Result** | One classification with reasoning | List of sections with assigned pages |
...
- Extracting assigned pages for further processing
