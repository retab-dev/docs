...
- **Consensus Scoring**: Increase `n_consensus` to get confidence signals like `likelihood` and `votes`
- **Config Generation**: Use `generate_split_config` to bootstrap subdocuments from a sample file
...
<ParamField body="n_consensus" type="integer">
  Number of split passes to run before building the final answer. Leave it at `1` for the fastest deterministic pass, or raise it when boundary quality is business-critical and you want `likelihood` and `votes` in the response.
</ParamField>
...
A SplitResponse object containing the classified sections with their assigned pages.
  <Expandable title="properties">
    <ResponseField name="splits" type="array[SplitResult]">
      ...
      - `likelihood`: Confidence score for this split when `n_consensus > 1`
...
If you do not already know the right subdocument list, start with `generate_split_config` and then pass the generated `subdocuments` into `split`.
...
config = client.documents.generate_split_config(
...
result = client.documents.split(
    document="property_portfolio.pdf",
    model="retab-small",
    subdocuments=[subdoc.model_dump() for subdoc in config.subdocuments],
    n_consensus=3,
)

for split in result.splits:
    print(split.name, split.pages, split.likelihood)
```
...
const config = await client.documents.generate_split_config({
  document: "property_portfolio.pdf",
  model: "retab-small",
});
...
for (const split of result.splits) {
  console.log(split.name, split.pages, split.likelihood);
}
```
</CodeGroup>
