...
A SplitResponse object containing the classified sections with their assigned pages.
  <Expandable title="properties">
    ...
    <ResponseField name="consensus" type="SplitConsensus | null">
      ...
      - `likelihoods`: A likelihood tree that mirrors `splits`, with scores for each split name, page leaf, partition key, and partition page leaf
      - `choices`: The consolidated output at index `0`, then one entry per consensus run at indices `1..n`
    </ResponseField>
  </Expandable>
</ResponseField>
...
#   Invoice INV-003:
#     Pages: [2]
# {'splits': [{'name': 0.97, 'pages': [1.0, 0.96], 'partitions': [{'key': 0.99, 'pages': [1.0]}, {'key': 0.95, 'pages': [0.95, 0.95]}, {'key': 0.92, 'pages': [0.92]}]}]}
```
