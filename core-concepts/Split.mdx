...
<ParamField body="categories" type="array[Category]" required>
  ...
  - `partition_key` (optional): A key to further partition this category. When specified, the API will identify individual items within pages belonging to this category (e.g., "property ID" to find individual properties within a real estate document, or "invoice number" to separate multiple invoices)
</ParamField>
...
A SplitResponse object containing the classified sections with their page ranges.
  <Expandable title="properties">
    <ResponseField name="splits" type="array[SplitResult]">
      ...
      - `partitions`: List of sub-page partitions (populated when `partition_key` is specified on the category)
...
# invoice: pages [1, 2, 3]
# receipt: pages [4, 5]
# invoice: pages [6, 7, 8]  <- Same category, different location
# receipt: pages [9, 10]
...
When processing documents that contain multiple items of the same type (e.g., multiple invoices, multiple property listings), use the `partition_key` parameter to identify and separate individual items within a category.
...
# Define categories with partition_key for sub-document splitting
categories = [
    {
...
# Access partitions for each category
for split in result.splits:
    print(f"{split.name}: pages {split.pages}")
