...
A SplitResponse object containing the classified sections with their page ranges.
  <Expandable title="properties">
    <ResponseField name="splits" type="array[SplitResult]">
      ...
      - `partitions`: List of sub-page partitions for precise content boundaries
...
result = client.documents.split(
    document="multi_section_page.pdf",
    model="retab-small",
    categories=[
        {"name": "header", "description": "Document header and title section"},
        {"name": "body", "description": "Main content body"},
        {"name": "footer", "description": "Footer with page numbers and legal text"},
    ]
)
...
for split in result.splits:
    ...
    for partition in split.partitions:
        print(f"  Partition pages: {partition.pages}")
