...
- `client.edit.agent.fill(...)` for agent-based filling with automatic field detection
- `client.edit.templates.generate(...)` to infer a reusable PDF form schema
- `client.edit.templates.fill(...)` to fill a saved template
- `client.edit.templates.list(...)`, `get(...)`, `create(...)`, `update(...)`, `duplicate(...)`, and `delete(...)` to manage templates
- `client.documents.edit(...)` if you prefer a single request shape that supports either `document` or `template_id`

<Note>
Python exposes `color` as a top-level argument on `edit.agent.fill(...)` and `edit.templates.fill(...)`.

In the Node SDK, `edit.agent.fill(...)` uses `config: { color: "#000080" }`, while `edit.templates.fill(...)` accepts `color` directly.
</Note>
...
- **Template Management** (`edit.templates.*`): Create and reuse saved PDF templates for batch processing
...
for field in response.form_schema.form_fields:
    ...
    print(
        f"    Position: page {field.bbox.page}, "
        f"left={field.bbox.left:.3f}, top={field.bbox.top:.3f}"
    )
...
template = client.edit.templates.get(template_id="edittplt_abc123")
...
# Delete a template
client.edit.templates.delete(template_id="edittplt_abc123")
```
...
// Get a specific template
const fetchedTemplate = await client.edit.templates.get("edittplt_abc123");

// Update a template
const updated = await client.edit.templates.update("edittplt_abc123", {
    name: "Updated Form Name"
});
