...
Both flows share one size limit: **2 GiB (2,147,483,648 bytes) per document**. That is the cap on a remote document Retab fetches for you, and the cap on a file you upload to Retab directly.

Retab fetches a remote document into memory before storing it, so very large fetches are processed a small number at a time per server. A request for a multi-gigabyte document may therefore be rejected as temporarily unavailable while another large fetch is in progress; retry it. Files you upload to Retab yourself go straight to storage and are not affected.
