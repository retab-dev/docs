...
| Method | Purpose |
| ------ | ------- |
| **`upload`** | Upload a document and receive a persistent `MIMEData` reference for future requests. |
...
The returned `url` has the form `https://storage.retab.com/file_...`. It is an opaque Retab URL, not a public signed URL, and can be passed to later `MIMEData` requests without sending the file bytes again.

## Upload once, reuse by URL

For larger documents, upload the file first and pass the returned Retab storage URL to classify, split, extract, or workflow calls. This avoids sending the document bytes inline in every API request.
