...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut request = http.put(&session.upload_url).body(bytes);
...
var session = await client.Files.UploadAsync(
    client.ApiKey,
    new FilesUploadOptions
    {
...
var client = new Retab("YOUR_RETAB_API_KEY");
...
func main() {
	...
	_, err = client.Extractions.Create(ctx, retab.ExtractionCreateRequest{
...
// List recent files
var files = await client.Files.ListAsync(client.ApiKey, new FilesListOptions { Limit = 20 });
...
var link = await client.Files.GetDownloadLinkAsync("file_a1b2c3d4e5f6", client.ApiKey);
