...
Chunking now uses a simplified, predictable pricing rule based on document pages (not item count).
...
This means:
- Billing is independent of how many items were found in arrays
- Billing is the same whether routing is sparse or dense
- Chunking creates a single consolidated processing log with `source="parallel_pages_x2"`
