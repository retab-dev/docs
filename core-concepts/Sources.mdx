...
func main() {
	...
	sources, err := client.Extractions.Sources(ctx, result.ID)
	if err != nil {
		log.Fatal(err)
	}

	for field, leaf := range sources.Sources {
...
var mySchema = new Dictionary<string, object>
{
    ["type"] = "object",
...
// 2. Get sources for the extraction
var sources = await client.Extractions.SourcesAsync(result.Id);

// 3. Use anchors to highlight values in a PDF viewer
foreach (var kv in sources.Sources)
{
    var leaf = JObject.FromObject(kv.Value);
    var anchor = leaf["source"]?["anchor"] as JObject;
    if ((string?)anchor?["kind"] == "pdf_bbox")
    {
        Console.WriteLine(
