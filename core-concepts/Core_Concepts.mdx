...
Retab provides intelligent model routing through three special model identifiers: `retab-large`, `retab-small` and `retab-micro`. 
...
async def parse_document(document):
    client = AsyncRetab()
    result = await client.parses.create(document=document)
...
async function parseDocument(document) {
    ...
    const result = await client.parses.create({ document });
...
client.parses.create({ document: "invoice.pdf" })
...
func main() {
	...
	response, err := client.Extractions.Create(ctx, retab.ExtractionCreateRequest{
		Document:   "booking_confirmation.jpg",
		Model:      "retab-micro",
		JSONSchema: jsonSchema,
		NConsensus: 10, // This will run and combine the results of 10 calls to the same LLM
	})
	if err != nil {
		log.Fatal(err)
	}
