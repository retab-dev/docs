...
We use a standard JSON Schema with custom annotations (`X-SystemPrompt`, `X-LLMDescription`, and `X-ReasoningDescription`) to provide enhanced control over the extraction process. 
...
---
## What if I want to use my own client?

If you prefer to use your own OpenAI client locally (for example, to have more control over the API calls or to use a specific configuration), you can use the `preprocess` function instead of `extract`. 
This function returns the messages and schema ready to be used with OpenAI's API.
...
# Get the preprocessed messages and schema from UiForm
client = UiForm()
...
```
```python ANTHROPIC_CLIENT
import json
from uiform.client import UiForm
from anthropic import Anthropic

with open("tldr/tldr_email_schema.json", "r") as f:
    json_schema = json.load(f)

model = "claude-3-5-sonnet-20241022"

document = "tldr/tldr_07_01_25.eml"

uiform_client = UiForm()
messages, anthropic_schema = uiform_client.documents.preprocess(
    json_schema = json_schema,
    document = document,
    model = model,
    modality="native"
...
from uiform.jsonschema.utils import object_format_coercion
from jiter import from_json

result_json = response.content[0].text  # type: ignore
result_json = "{" + result_json  # We prefill the assistant with a curly bracket

try:
    final_object = from_json(bytes(result_json, "utf-8"), partial_mode=False)
    final_object = object_format_coercion(final_object, anthropic_schema)
...
This approach gives you full control over the OpenAI API call while still benefiting from UiForm's document preprocessing capabilities.

---

## Go further

- [Prompt Engineering Guide](https://docs.uiform.com/get-started/prompt-engineering-guide)
