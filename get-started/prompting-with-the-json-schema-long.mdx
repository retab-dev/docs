...
2. **`X-FieldPrompt`**: If present, it replaces the current property description with a more detailed one, helping the LLM better understand the property and extract it accurately.
3. **`X-ReasoningPrompt`**: For fields that are harder to extract, this annotation generates a sibling field of type `string` to act as a reasoning sandbox. This helps the LLM provide better outputs since it is a language-based model.

### Examples of `X-ReasoningPrompt`
...
```json
{
  "$defs": {
    "ArrayItem": {
      "X-ReasoningPrompt": "This is the Array Item reasoning description",
      "properties": {
        "array_item_string_field": {
          "X-ReasoningPrompt": "This is the array_item_string_field reasoning description",
    ...
    "SimpleObject": {
      "properties": {
        ...
        "object_string_field": {
          "X-ReasoningPrompt": "This is the object_string_field reasoning description",
  ...
  "X-ReasoningPrompt": "This is the root reasoning description",
  "X-SystemPrompt": "This is the system prompt",
  "properties": {
    "my_string_field": {
      "X-ReasoningPrompt": "This is the my_string_field reasoning description",
    ...
    "my_simple_object": {
      ...
      "X-ReasoningPrompt": "This is the my_simple_object reasoning description",
      "description": "my_simple_object description"
    },
    "my_array": {
      "X-ReasoningPrompt": "This is my_array reasoning description",
...
class BasicBaseModel(BaseModel):
    ...
    class SimpleObject(BaseModel):
        object_integer_field: int = Field(..., description="Simple integer field description",
                                          json_schema_extra={"X-FieldPrompt": "This is the object integer field LLM description"})
        object_string_field: str = Field(...,
                                         description="Simple string field description",
                                         json_schema_extra={"X-ReasoningPrompt": "This is the object string field reasoning description"})

    class ArrayItem(BaseModel):
        model_config = ConfigDict(json_schema_extra = {"X-ReasoningPrompt": "This is the Array Item reasoning description"})
    ...
    my_array: list[ArrayItem] = Field(...,
                                      ...
                                      json_schema_extra={"X-ReasoningPrompt": "This is the array reasoning description"}
                                      )
...
The content of `path_to_file.json` is actually the schema from [Examples of X-ReasoningPrompt](#examples-of-X-ReasoningPrompt).
...
- **`X-FieldPrompt`:** Provide detailed context for fields requiring nuanced interpretation. Example:  
  ```json
  {
    "type": "string",
    "X-FieldPrompt": "The name of the client as it appears on the official invoice."
  }
  ```
- **`X-ReasoningPrompt`:** Use this for fields where reasoning or additional context can clarify ambiguity. Example:  
  ```json
  {
    "type": "string",
    "X-ReasoningPrompt": "Explain how the extracted name matches the format required by the client's database."
  }
  ```
...
#### 2. Field-Specific Instructions (`X-FieldPrompt`)
...
```json
{
  ...
  "properties": {
    "invoice_number": {
      "type": "string",
      "X-FieldPrompt": "The unique identifier for the invoice, typically alphanumeric."
    },
    "date_issued": {
      "type": "string",
      "X-FieldPrompt": "The date when the invoice was issued, in YYYY-MM-DD format."
    }
  },
...
#### 3. Giving the Model Time to Think (`X-ReasoningPrompt`)
...
```json
{
  ...
  "properties": {
    "client_name": {
      "type": "string",
      "X-ReasoningPrompt": "Provide reasoning for how this name matches the provided document."
    },
    "amount_due": {
      "type": "number",
      "X-ReasoningPrompt": "Explain how the amount was calculated based on the invoice details."
    }
  }
}
```
...
```json
{
  ...
  "properties": {
    "invoice_number": {
      "type": "string",
      "X-FieldPrompt": "Unique invoice identifier."
    },
    "date_issued": {
      "type": "string",
      "X-ReasoningPrompt": "Explain how the date was identified in the document."
    }
  },
  "required": ["invoice_number", "date_issued"]
}
...
class InvoiceSchema(BaseModel):
    ...
    invoice_number: str = Field(..., description="Unique identifier for the invoice",
                                json_schema_extra={"X-FieldPrompt": "Unique invoice identifier."})
    date_issued: str = Field(..., description="Date in YYYY-MM-DD format",
                             json_schema_extra={"X-ReasoningPrompt": "Explain how the date was identified in the document."})
