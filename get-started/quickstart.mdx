...
<Steps>
  ...
  <Step title="Create your deployment">
    Configure an deployment (mailbox or link) that will automatically process incoming documents using your schema and send results to your webhook
  </Step>
  <Step title="Test your deployment">
    Validate your setup by sending test documents through your deployment and verify the extracted data matches your requirements
  </Step>
</Steps>
...
Then, as we will use your API key to make requests to OpenAI on your behalf within an deployment, you need to store your API key in the UiForm secrets manager:
...
We use a standard JSON Schema with custom annotations (`X-SystemPrompt`, `X-ReferenceQuote` and `X-ReasoningPrompt`) as a prompt-engineering framework for the extraction process.
...
Finally, integrate the webhook with your deployment system using the `uiform` client. This example demonstrates how to create an deployment that triggers the webhook when a matching event occurs:
...
deployment = uiclient.deployments.mailboxes.create(
    email="invoices@mailbox.uiform.com",
    model="gpt-4.1-nano",
    json_schema=Invoice.model_json_schema(), # use the pydantic model to create the json schema
    webhook_url="https://your-server.com/webhook",  # Replace with your actual webhook URL (taken from Step 3.1 for example)
)

```

At any email sent to `invoices@mailbox.uiform.com`, the deployment will send a POST request to your FastAPI webhook endpoint, where the payload can be processed.

You can see the deployment you just created on your [dashboard](https://www.uiform.com/dashboard/deployments) !

### Step 5: Test your deployment

Finally, you can test the deployment rapidly with the test functions of the sdk: 
...
<Tip>You can also test your webhook locally by overriding the webhook url set in the deployment</Tip>

You can also test your deployment directly from the [dashboard](https://www.uiform.com/dashboard/deployments):
<Frame>
...
log = uiclient.deployments.mailboxes.tests.webhook(
    email="test-mailbox-local@devmail.uiform.com", 
    webhook_url="http://localhost:8000/webhook" # If you want to try your webhook locally, you can override the webhook url set in the deployment
)
```
