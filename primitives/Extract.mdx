...
func main() {
	...
	if consensus, ok := (*extraction)["consensus"].(map[string]any); ok {
...
var sources = await client.Extractions.SourcesAsync("extr_01G34H8J2K", client.ApiKey);
...
func main() {
	...
	recent, err := client.Extractions.List(ctx, &retab.ListExtractionsParams{
	...
	customerSpecific, err := client.Extractions.List(ctx, &retab.ListExtractionsParams{
	...
	dateFiltered, err := client.Extractions.List(ctx, &retab.ListExtractionsParams{
		ListParams: retab.ListParams{FromDate: &from, ToDate: &to},
	})
	if err != nil {
		log.Fatal(err)
	}
...
foreach (var e in recent.Items) Console.WriteLine(e.Id);
...
var customerSpecific = await client.Extractions.ListAsync(
    ...
    {
        Limit = 50,
        Metadata = new Dictionary<string, string> { ["customer_id"] = "cust_acme_corp" },
    }
);
foreach (var e in customerSpecific.Items) Console.WriteLine(e.Id);
...
foreach (var e in dateFiltered.Items) Console.WriteLine(e.Id);
```
...
var extraction = await client.Extractions.GetAsync("extr_01G34H8J2K", client.ApiKey);
Console.WriteLine(extraction.Id);
foreach (var kv in extraction.Output)
{
    Console.WriteLine($"{kv.Key}: {kv.Value}");
...
foreach (var e in customerExtractions.Items)
{
...
func main() {
	...
	extractedData, _ := (*result)["output"].(map[string]any)
	likelihoods := map[string]any{}
	if consensus, ok := (*result)["consensus"].(map[string]any); ok {
		if l, ok := consensus["likelihoods"].(map[string]any); ok {
			likelihoods = l
		}
	}
	...
	if usage, ok := (*result)["usage"].(map[string]any); ok {
		fmt.Printf("Processed with %v tokens\n", usage["total_tokens"])
	}
}
```
...
if (likelihoods != null && likelihoods.Count > 0 && likelihoods.Values.All(score => score > 0.7))
{
...
if (result.Usage != null)
{
    Console.WriteLine($"Processed with {result.Usage.TotalTokens} tokens");
}
```
