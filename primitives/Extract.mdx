...
func main() {
	...
	orgSpecific, err := client.Extractions.List(ctx, &retab.ListExtractionsParams{
		ListParams: retab.ListParams{Limit: 50},
		Metadata:   map[string]string{"organization_id": "org_acme_corp"},
	})
	if err != nil {
		log.Fatal(err)
	}
	fmt.Println(orgSpecific.Data)
...
func main() {
	...
	orgExtractions, err := client.Extractions.List(ctx, &retab.ListExtractionsParams{
	...
	fmt.Println(orgExtractions.Data)
}
```
</CodeGroup>
...
func main() {
	...
	tenantExtractions, err := client.Extractions.List(ctx, &retab.ListExtractionsParams{
		ListParams: retab.ListParams{Limit: 50},
		Metadata: map[string]string{
			"organization_id": "org_123",
			"source":          "sinari_jobs",
	...
	fmt.Println(tenantExtractions.Data)
}
```
