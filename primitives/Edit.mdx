...
<Note>
  In both SDKs, `color` is a top-level argument on `edits.create(...)` and
...
if result.data.filled_document: # Extract base64 content from data URI
