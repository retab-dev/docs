...
<ParamField body="document" type="MIMEData" required>
  ...
  content (e.g., "data:application/pdf;base64,...")
</ParamField>
...
if result.output.filled_document: # Extract base64 content from data URI
...
if (result.output.filled_document) {
    const base64Content = result.output.filled_document.url.split(",")[1];
...
// Review what was filled
console.log(`Filled ${result.output.form_data.length} form fields:`);
result.output.form_data.forEach(field => {
    if (field.value) {
...
func main() {
	...
	result, err := client.Edits.Create(ctx, retab.EditCreateRequest{
		Document: document,
		Instructions: `
        Full Name: Jane Smith
        Date of Birth: March 15, 1985
	...
	filled, _ := output["filled_document"].(map[string]any)
	if url, ok := filled["url"].(string); ok && url != "" {
	...
	formData, _ := output["form_data"].([]any)
	fmt.Printf("Filled %d form fields:\n", len(formData))
	for _, raw := range formData {
		field, _ := raw.(map[string]any)
		if value, ok := field["value"]; ok && value != nil && value != "" {
			fmt.Printf("  - %v: %v\n", field["key"], value)
		}
	}
}
		...
		FormFields: []retab.FormField{
			{
				...
				"description": "Applicant's full legal name",
				"type":        "text",
				"bbox":        map[string]any{"left": 0.15, "top": 0.12, "width": 0.35, "height": 0.03, "page": 1},
			},
			{
				"key":         "date_of_birth",
				"description": "Date of birth, ISO 8601",
				"type":        "text",
				"bbox":        map[string]any{"left": 0.15, "top": 0.18, "width": 0.25, "height": 0.03, "page": 1},
			},
		},
	})
	if err != nil {
...
# For batch processing, use edits.create(template_id=...) with a pre-created template
...
func main() {
	...
	// For batch processing, use Edits.Templates.Fill with a pre-created template.
	// This skips field detection for each document, making it much faster.
	for i, person := range applicants {
		...
		filled, _ := data["filled_document"].(map[string]any)
		if url, ok := filled["url"].(string); ok && url != "" {
			parts := strings.SplitN(url, ",", 2)
			if len(parts) == 2 {
				filledBytes, err := base64.StdEncoding.DecodeString(parts[1])
