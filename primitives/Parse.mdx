...
<ParamField body="image_resolution_dpi" type="integer" default="192">
  DPI used when rasterizing pages for OCR-backed parsing. Accepted values are
  `96` to `300`.
</ParamField>
...
{
"page": page_num,
"chunk_id": f"page*{page*num}\_chunk*{chunk_idx}",
"text": str(chunk),
...
func main() {
	...
	file, _ := (*parse)["file"].(map[string]any)
	filename, _ := file["filename"].(string)
	usage, _ := (*parse)["usage"].(map[string]any)
	pageCount, _ := usage["page_count"].(float64)
...
console.log(
  `Created ${allChunks.length} chunks from ${parse.usage.page_count} pages`,
);
console.log(parse.output.text);
```
