...
<ParamField body="subdocuments" type="array[Subdocument]" required>
  ...
  - `partition_key` (optional): Key used to partition repeated instances inside this subdocument
  - `allow_overlap` (optional, default `true`): Set to `false` when partition chunks for this subdocument must be exclusive
