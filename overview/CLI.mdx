...
1. **Flag**: `--api-key sk_live_…` on a single command.
2. **Env**: `RETAB_API_KEY=sk_live_…` exported in the shell or CI runner.
3. **Stored**: `retab auth login --api-key sk_live_…` to persist to `~/.retab/config.json`.
...
Get a key from the [dashboard settings](https://retab.com/dashboard/settings).
...
retab auth login --api-key sk_live_…      # API key — overwrites any stored OAuth tokens
...
printf '%s\n' '{"review":{"predicate":{"kind":"any_required_field_null"},"skip_in_test_mode":false}}' \
  > extract-review.json
...
printf '%s\n' '{"review":{"predicate":{"kind":"boundary_confidence_lt","threshold":0.8},"skip_in_test_mode":false}}' \
...
retab workflows reviews get <run-id> <block-id>
retab workflows reviews approve <run-id> <block-id> --version-id <version-id>

# Append a corrected output snapshot, then approve its returned version id.
retab workflows reviews edit <run-id> blk_for_each_1 \
  --parent-id <version-id> \
  --snapshot-file ./corrected-output.json
retab workflows reviews approve <run-id> blk_for_each_1 --version-id <new-version-id>
