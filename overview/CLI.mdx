...
# Human review is a gate on a block config, not a separate block.
...
# Runs paused by a gate use waiting_for_human.
...
retab workflows reviews approve <run-id> <block-id> --version-stamp 0

# Approve a corrected split_by_key for_each partition map inline.
retab workflows reviews for-each approve <run-id> blk_for_each_1 \
  --version-stamp 0 \
