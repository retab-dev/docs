...
| `workflows_steps_get`  | Inspect a single step. For extract blocks, surfaces the top-level `extraction_id` so you can pipe it straight into `files_get_sources`. Supports `path` to drill into big payloads.                        |
