...
A document's `url` may be a `data:` URL carrying inline base64 content, a Retab
storage URL (`https://storage.retab.com/...`) returned by the file upload
endpoint, or an HTTPS URL on one of the supported object-storage hosts listed in
[Files](/core-concepts/Files) — Azure Blob Storage, Google Cloud Storage, Amazon
S3 and Cloudflare R2 — which Retab fetches server-side. URLs on any other host
are rejected with a `400`; upload the file to Retab first and pass the returned
storage URL instead.
