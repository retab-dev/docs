---
title: 'Get Run'
openapi: 'GET /v1/workflows/runs/{run_id}'
---

Get a single workflow run by ID. Use this endpoint to check the status of a running workflow or retrieve the results of a completed workflow.

`run.steps` contains execution status for each block and artifact refs at `step.execution.artifacts`. For typed handle inputs and outputs, use [Get Step](/api-reference/workflows/runs/steps/get).

<RequestExample>

```python python
from retab import Retab

client = Retab()

run = client.workflows.runs.get("run_abc123xyz")

print(f"Status: {run.status}")
if run.status == "completed":
    print(f"Duration: {run.duration_ms}ms")
    print(f"Outputs: {run.final_outputs}")
```

```javascript javascript
import Retab from 'retab';

const client = new Retab();

const run = await client.workflows.runs.get("run_abc123xyz");

console.log(`Status: ${run.status}`);
if (run.status === "completed") {
    console.log(`Duration: ${run.durationMs}ms`);
    console.log("Outputs:", run.finalOutputs);
}
```

```bash curl
curl -X 'GET' \
  'https://api.retab.com/v1/workflows/runs/run_abc123xyz' \
  -H 'Api-Key: <your-api-key>'
```

</RequestExample>


<ResponseExample>

```json 200
{
  "id": "run_abc123xyz",
  "workflow_id": "wf_abc123xyz",
  "workflow_name": "Invoice Processing",
  "organization_id": "org_123",
  "status": "completed",
  "started_at": "2024-01-15T10:30:00Z",
  "completed_at": "2024-01-15T10:30:15Z",
  "duration_ms": 15000,
  "steps": [
    {
      "block_id": "start-block-1",
      "block_type": "start",
      "block_label": "Invoice Input",
      "status": "completed",
      "execution": {
        "artifacts": []
      },
      "started_at": "2024-01-15T10:30:00Z",
      "completed_at": "2024-01-15T10:30:01Z",
      "duration_ms": 1000
    },
    {
      "block_id": "extract-block-1",
      "block_type": "extract",
      "block_label": "Extract Data",
      "status": "completed",
      "execution": {
        "artifacts": [{ "operation": "extraction", "id": "ext_abc123" }]
      },
      "started_at": "2024-01-15T10:30:01Z",
      "completed_at": "2024-01-15T10:30:10Z",
      "duration_ms": 9000
    },
    {
      "block_id": "end-block-1",
      "block_type": "end",
      "block_label": "Output",
      "status": "completed",
      "execution": {
        "artifacts": []
      },
      "started_at": "2024-01-15T10:30:10Z",
      "completed_at": "2024-01-15T10:30:15Z",
      "duration_ms": 5000
    }
  ],
  "input_documents": {
    "start-block-1": {
      "id": "file_123",
      "filename": "invoice.pdf",
      "mime_type": "application/pdf"
    }
  },
  "final_outputs": {
    "end-block-1": {
      "document": null,
      "data": {
        "invoice_number": "INV-2024-001",
        "total_amount": 1234.56,
        "vendor_name": "Acme Corp"
      }
    }
  },
  "error": null,
  "created_at": "2024-01-15T10:30:00Z",
  "updated_at": "2024-01-15T10:30:15Z",
  "waiting_for_block_ids": []
}
```

```json 404
{
  "detail": "Workflow run not found"
}
```

</ResponseExample>
