---
title: "List Workflows"
openapi: "GET /v1/workflows"
---

List workflows with id pagination. Use `before` or `after` to page through results, and `fields` to limit the returned fields.

<RequestExample>

```python python
from retab import Retab

client = Retab()

workflows = client.workflows.list(
    limit=10,
    order="desc",
    sort_by="updated_at",
)

filtered = client.workflows.list(
    after="workflow_abc123xyz",
    limit=5,
    fields="id,name,updated_at",
)

print(workflows.list_metadata.after)
print(filtered.data[0]["name"])
```

```javascript javascript
import { Retab } from "@retab/node";

const client = new Retab();

const workflows = await client.workflows.list({
  limit: 10,
  order: "desc",
  sortBy: "updated_at",
});

const filtered = await client.workflows.list({
  after: "workflow_abc123xyz",
  limit: 5,
  fields: "id,name,updated_at",
});

console.log(workflows.list_metadata.after);
console.log(filtered.data[0].name);
```

```go go
package main

import (
	"context"
	"fmt"
	"log"

	retab "github.com/retab-dev/retab/clients/go"
)

func main() {
	ctx := context.Background()

	client, err := retab.NewClient("")
	if err != nil {
		log.Fatal(err)
	}

	workflows, err := client.Workflows.List(ctx, &retab.ListWorkflowsParams{
		Limit:  10,
		Order:  "desc",
		SortBy: "updated_at",
	})
	if err != nil {
		log.Fatal(err)
	}

	filtered, err := client.Workflows.List(ctx, &retab.ListWorkflowsParams{
		After:  "workflow_abc123xyz",
		Limit:  5,
		Fields: "id,name,updated_at",
	})
	if err != nil {
		log.Fatal(err)
	}

	fmt.Println(workflows.ListMetadata.After)
	fmt.Println(filtered.Data[0].Name)
}
```

```bash curl
curl -X 'GET' \
  'https://api.retab.com/v1/workflows?limit=10&order=desc&sort_by=updated_at' \
  -H 'accept: application/json' \
  -H 'Api-Key: <your-api-key>'

curl -X 'GET' \
  'https://api.retab.com/v1/workflows?after=workflow_abc123xyz&limit=5&fields=id%2Cname%2Cupdated_at' \
  -H 'accept: application/json' \
  -H 'Api-Key: <your-api-key>'
```

</RequestExample>

<ResponseExample>

```json 200
{
  "data": [
    {
      "id": "workflow_def456xyz",
      "name": "Invoice Processing",
      "description": "Extract invoice fields and route for review",
      "published": {
        "version_id": "ver_8zJfV7T9qK2mP4xN6bR1cD3eF5gH7iJ9",
        "published_at": "2026-03-12T09:45:00Z"
      },
      "created_at": "2026-03-10T12:00:00Z",
      "updated_at": "2026-03-12T09:45:00Z"
    },
    {
      "id": "workflow_abc123xyz",
      "name": "Receipt Classification",
      "description": "",
      "published": null,
      "created_at": "2026-03-09T15:30:00Z",
      "updated_at": "2026-03-11T18:20:00Z"
    }
  ],
  "list_metadata": {
    "before": null,
    "after": "workflow_abc123xyz"
  }
}
```

</ResponseExample>
