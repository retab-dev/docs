...
openapi: 'GET /v1/workflows/{workflow_id}/tests/{test_id}/runs/{run_id}'
---

Fetch a single immutable run record. Use this after polling
[Execute Workflow Tests](../execute) to inspect the inputs the block ran with,
...
```bash curl
curl -X 'GET' \
  'https://api.retab.com/v1/workflows/wf_abc123xyz/tests/wfnodetest_hsLEQiM61ez9Piv147MWk/runs/wfnodetestrun_tRNzPiMpBas4kDwD74M8d' \
...
```json 404
{
  "detail": "Workflow test run not found: wfnodetestrun_tRNzPiMpBas4kDwD74M8d"
}
```
