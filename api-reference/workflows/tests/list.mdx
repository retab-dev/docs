...
func main() {
	...
	// All tests for a workflow.
	tests, err := client.Workflows.Tests.List(ctx, &retab.WorkflowTestsListParams{
		WorkflowID: "wf_abc123xyz",
	...
	// Only tests for one specific block.
	testsForBlock, err := client.Workflows.Tests.List(ctx, &retab.WorkflowTestsListParams{
		WorkflowID:    "wf_abc123xyz",
...
# All tests for a workflow.
tests = client.workflow_tests.list(
  workflow_id: 'wf_abc123xyz',
  limit: 50,
)

# Only tests for one specific block.
tests_for_block = client.workflow_tests.list(
  workflow_id: 'wf_abc123xyz',
  target_block_id: 'block_extract_invoice',
  limit: 20,
)

puts "Workflow has #{tests.data.length} tests"
```

```rust Rust
use retab::resources::workflow_tests::ListParams;
use retab::Retab;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Workflow has {} tests", tests.data.len());
    Ok(())
}
```
...
$result = $client->workflowTests()->list(
...
var result = await client.Workflows.Tests.ListAsync(new WorkflowTestsListOptions());
...
public final class Example {
  public static void main(String[] args) throws Exception {
    ...
    var result = client.workflows().tests().list("wf_abc123", null, null, null, 10L, null);
