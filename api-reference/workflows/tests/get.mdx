---
title: "Get Workflow Test"
openapi: "GET /v1/workflows/tests/{test_id}"
---

Fetch a single workflow test by id. The response refreshes the test's
`validation_status` and schema-drift state against the current workflow draft
before returning, so the caller always sees the latest staleness.

<RequestExample>

```python Python
from retab import Retab

client = Retab()

test = client.workflows.tests.get(
    test_id="wfnodetest_hsLEQiM61ez9Piv147MWk",
)

print(test.assertion.condition)
print(test.schema_drift)
```

```typescript TypeScript
import { Retab } from "@retab/node";

const client = new Retab({ apiKey: process.env.RETAB_API_KEY });

const test = await client.workflows.tests.get("wfnodetest_hsLEQiM61ez9Piv147MWk");

console.log(test.assertion.condition);
console.log(test.schemaDrift);
```

```go Go
package main

import (
	"context"
	"fmt"
	"log"

	retab "github.com/retab-dev/retab/clients/go"
)

func ptr[T any](v T) *T { return &v }

func main() {
	ctx := context.Background()

	client, err := retab.NewClient("")
	if err != nil {
		log.Fatal(err)
	}

	test, err := client.Workflows.Tests.Get(ctx, "wfnodetest_hsLEQiM61ez9Piv147MWk")
	if err != nil {
		log.Fatal(err)
	}

	fmt.Println(test.Assertion)
	fmt.Println(test.SchemaDrift)
}
```

```ruby Ruby
require 'retab'

client = Retab::Client.new(api_key: ENV['RETAB_API_KEY'])

test = client.workflows.tests.get(test_id: 'wfnodetest_hsLEQiM61ez9Piv147MWk')

puts test.assertion.condition
puts test.schema_drift
```

```rust Rust
use retab::Retab;

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let client = Retab::new(std::env::var("RETAB_API_KEY")?);

    let test = client
        .workflows().tests()
        .get("wfnodetest_hsLEQiM61ez9Piv147MWk")
        .await?;
    if let Some(assertion) = test.assertion.as_ref() {
        println!("{:?}", assertion.condition);
    }
    println!("{:?}", test.schema_drift);
    Ok(())
}
```

```php PHP
<?php
require 'vendor/autoload.php';

use Retab\Client;

$client = new Client(apiKey: getenv('RETAB_API_KEY'));

$result = $client->workflows()->tests()->get(
    testId: 'test_abc123',
);
print_r($result);
```

```csharp C#
using Retab;
using RetabClient = Retab.Retab;

var apiKey = Environment.GetEnvironmentVariable("RETAB_API_KEY")!;
var client = new RetabClient(apiKey);

var result = await client.Workflows.Tests.GetAsync("test_abc123");
Console.WriteLine(result);
```

```java Java
import com.retab.RetabClient;

public final class Example {
  public static void main(String[] args) throws Exception {
    RetabClient client = new RetabClient(System.getenv("RETAB_API_KEY"));

    var result = client.workflows().tests().get("test_abc123");
    System.out.println(result);
  }
}
```

```curl cURL
curl -X 'GET' \
  'https://api.retab.com/v1/workflows/tests/wfnodetest_hsLEQiM61ez9Piv147MWk' \
  -H 'accept: application/json' \
  -H 'Authorization: Bearer <your-api-key>'
```

</RequestExample>

<ResponseExample>

```json 200
{
  "id": "wfnodetest_hsLEQiM61ez9Piv147MWk",
  "workflow_id": "wf_abc123xyz",
  "target": { "type": "block", "block_id": "block_extract_invoice" },
  "source": {
    "type": "manual",
    "handle_inputs": {
      "input-document-0": { "type": "file", "file_id": "file_invoice_q1" }
    }
  },
  "name": "Q1 invoice total",
  "assertion": {
    "id": "assert_xyz",
    "target": { "output_handle_id": "output-json-0", "path": "total" },
    "condition": { "kind": "equals", "expected": 1234.56 },
    "label": null
  },
  "schema_drift": "none",
  "schema_drift_detail": null,
  "validation_status": "valid",
  "validation_issues": [],
  "latest_run_summary": {
    "run_record_id": "wfnodetestrun_abc123",
    "status": "passed",
    "started_at": "2026-05-01T13:55:00Z",
    "completed_at": "2026-05-01T13:55:18Z",
    "duration_ms": 18221,
    "workflow_draft_fingerprint": "ddd95baadce6045f",
    "block_config_fingerprint": "0ff93ddc7cefcb42",
    "assertions_passed": 1,
    "assertions_failed": 0,
    "blocked_assertions": 0
  },
  "latest_passing_run_summary": { "...": "as above" },
  "latest_failing_run_summary": null,
  "created_at": "2026-04-08T14:22:26Z",
  "updated_at": "2026-04-08T14:27:52Z"
}
```

```json 404
{
  "detail": "Workflow test not found: wfnodetest_hsLEQiM61ez9Piv147MWk"
}
```

</ResponseExample>
