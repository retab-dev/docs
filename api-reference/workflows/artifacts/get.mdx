...
step = client.workflows.steps.get("run_abc123xyz", "function-block-1")
...
func main() {
	...
	step, err := client.Workflows.Steps.Get(ctx, "run_abc123xyz", "function-block-1")
	if err != nil {
		log.Fatal(err)
	}

	if step.Artifact != nil {
		artifact, err := client.Workflows.Artifacts.GetRef(ctx, *step.Artifact)
		if err != nil {
			log.Fatal(err)
		}
		fmt.Println((*artifact)["operation"])
		fmt.Println((*artifact)["output"])
