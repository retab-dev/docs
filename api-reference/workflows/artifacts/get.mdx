...
Fetch a persisted workflow artifact record by operation and artifact ID.
...
artifact = client.workflows.artifacts.get(
    operation="function_invocation",
    artifact_id="finv_abc123",
)
...
func main() {
	...
	artifact, err := client.Workflows.Artifacts.Get(ctx, "function_invocation", "finv_abc123")
	if err != nil {
		log.Fatal(err)
	}
