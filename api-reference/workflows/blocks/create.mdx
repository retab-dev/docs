...
- Input blocks (`start-document`, `start_json`) cannot be placed inside containers — `parent_id` must be `null` for them.
...
const block = await client.workflows.blocks.create("wf_abc123xyz", {
  id: "extract-1",
  type: "extract",
  label: "Extract Invoice Fields",
  positionX: 320,
...
```json 400
{
  "detail": "Input blocks (start-document, start-json) cannot be placed inside containers"
}
```
