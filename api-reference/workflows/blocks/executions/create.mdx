---
title: "Create Workflow Block Execution"
openapi: "POST /v1/workflows/blocks/executions"
---

Create a workflow block execution from a prior run.

<RequestExample>

```python Python
from retab import Retab

client = Retab()

block_execution = client.workflows.blocks.executions.create(
    run_id="run_abc123xyz",
    block_id="blk_extract_1",
    step_id="step_iter_0_blk_extract_1",
    n_consensus=5,
)
```

```typescript TypeScript
import { Retab } from "@retab/node";

const client = new Retab({ apiKey: process.env.RETAB_API_KEY });

const block_execution = await client.workflows.blocks.executions.create("run_abc123xyz", "blk_extract_1", "step_iter_0_blk_extract_1", 5);
```

```go Go
block_execution, err := client.Workflows.Blocks.Executions.Create(ctx, retab.CreateWorkflowBlockExecutionRequest{
	RunID:      "run_abc123xyz",
	BlockID:    "blk_extract_1",
	StepID:     "step_iter_0_blk_extract_1",
	NConsensus: 5,
})
```

```ruby Ruby
require 'retab'

client = Retab::Client.new(api_key: ENV['RETAB_API_KEY'])

block_execution = client.workflow_block_executions.create(
  run_id: 'run_abc123xyz',
  block_id: 'blk_extract_1',
  step_id: 'step_iter_0_blk_extract_1',
  n_consensus: 5,
)
```

```rust Rust
use retab::models::CreateBlockExecutionRequest;
use retab::resources::workflow_block_executions::CreateParams;
use retab::Retab;

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let client = Retab::new(std::env::var("RETAB_API_KEY")?);

    let block_execution = client
        .workflow_block_executions()
        .create(CreateParams::new(CreateBlockExecutionRequest {
            run_id: "run_abc123xyz".into(),
            block_id: "blk_extract_1".into(),
            step_id: Some("step_iter_0_blk_extract_1".into()),
            n_consensus: Some(5),
            check_eligibility: None,
        }))
        .await?;

    println!("{}", block_execution.id);
    Ok(())
}
```

```php PHP
<?php
require 'vendor/autoload.php';

use Retab\Client;

$client = new Client(apiKey: getenv('RETAB_API_KEY'));

$result = $client->workflowBlockExecutions()->create(
    runId: 'run_abc123',
    blockId: 'blk_extract_1',
);
print_r($result);
```

```csharp C#
using Retab;

var apiKey = Environment.GetEnvironmentVariable("RETAB_API_KEY")!;
var client = new Retab(apiKey);

var result = await client.WorkflowBlockExecutions.CreateAsync(apiKey, new WorkflowBlockExecutionsCreateOptions());
Console.WriteLine(result);
```

```curl cURL
curl -X 'POST' \
  'https://api.retab.com/v1/workflows/blocks/executions' \
  -H 'Api-Key: <your-api-key>' \
  -H 'Content-Type: application/json' \
  -d '{"run_id":"run_abc123xyz","block_id":"blk_extract_1"}'
```

</RequestExample>

<ResponseExample>

```json 200
{
  "id": "wfsim_abc123",
  "run_id": "run_abc123xyz",
  "block_id": "blk_extract_1",
  "lifecycle": { "status": "completed" }
}
```

</ResponseExample>
