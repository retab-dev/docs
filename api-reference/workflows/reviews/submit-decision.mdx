...
| `reviewable_value` | Optional, `approved` only. Primitive-specific reviewed value, such as a split manifest or split-by-key `for_each` partition map. Mutually exclusive with `edited_output`. |
...
const overlay = await client.workflows.reviews.get("run_abc123", "block_extract");
...
// Approve with edits — server appends a corrective version, then approves it
await client.workflows.reviews.approve("run_abc123", "block_extract", {
  versionStamp,
