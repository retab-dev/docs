...
| Field | Notes |
|---|---|
| `snapshot` | The corrected output payload. Required unless `reviewable_value` is provided. |
| `reviewable_value` | Primitive-specific reviewed value, such as a split manifest or split-by-key `for_each` partition map. Mutually exclusive with `snapshot`. |
...
let overlay = await client.workflows.reviews.get("run_abc123", "block_extract");

try {
  overlay = await client.workflows.reviews.edit("run_abc123", "block_extract", {
    snapshot: { total_amount: 325, vendor_name: "Acme Corp" },
  ...
  if (err.status === 409) {
    // Overlay moved on — re-read and retry with the fresh stamp.
    overlay = await client.workflows.reviews.get("run_abc123", "block_extract");
  } else {
    throw err;
  }
}
