...
try {
  overlay = await client.workflows.runs.reviews.edit("run_abc123", "hil-1", {
    snapshot: { total_amount: 325, vendor_name: "Acme Corp" },
  ...
  if (err.status === 409) {
    // Overlay moved on — re-read and retry with the fresh stamp.
    overlay = await client.workflows.runs.reviews.get("run_abc123", "hil-1");
  } else {
    throw err;
  }
}
...
```bash curl
curl -X 'POST' \
  'https://api.retab.com/v1/workflows/reviews/run_abc123/hil-1/versions' \
