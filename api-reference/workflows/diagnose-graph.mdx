---
title: 'Diagnose Workflow Graph'
openapi: 'POST /v1/workflows/{workflow_id}/diagnose-graph'
---

Validate workflow blocks and edges before publishing. You can pass a draft
graph directly, or fetch it with [Get Workflow Entities](/api-reference/workflows/get-entities)
and forward it to this endpoint.

The check covers:

- Missing or duplicate start-document blocks
- Disconnected blocks
- Dangling edges (one endpoint missing)
- Unreachable blocks
- Type mismatches between source/target handles
- Missing required block configuration
- Incomplete authored config such as blank categories, subdocuments, or map keys
- Embedded human-review gate predicates and unavailable review signals
- Cycles in the directed graph

Severities:

| Severity | Meaning |
|---|---|
| `error` | Must fix before publish — block runs would fail. |
| `warning` | Should fix; runs may still succeed. |
| `info` | Advisory only. |

`is_valid` is `true` when the issue list contains no errors. `re_propagate`
defaults to `true` so derived schemas are refreshed before validation.

The SDKs also expose a convenience helper:

<RequestExample>

```python python
from retab import Retab

client = Retab()

diagnosis = client.workflows.diagnose("wf_abc123")
for issue in diagnosis.issues:
    print(f"[{issue.severity}] {issue.code}: {issue.message}")
```

```javascript javascript
import { Retab } from '@retab/node';

const client = new Retab();

const diagnosis = await client.workflows.diagnose("wf_abc123");
for (const issue of diagnosis.issues) {
  console.log(`[${issue.severity}] ${issue.code}: ${issue.message}`);
}
```

```go go
package main

import (
	"context"
	"fmt"
	"log"

	retab "github.com/retab-dev/retab/clients/go"
)

func main() {
	ctx := context.Background()

	client, err := retab.NewClient("")
	if err != nil {
		log.Fatal(err)
	}

	diagnosis, err := client.Workflows.Diagnose(ctx, "wf_abc123")
	if err != nil {
		log.Fatal(err)
	}

	for _, issue := range diagnosis.Issues {
		fmt.Printf("[%s] %s: %s\n", issue.Severity, issue.Code, issue.Message)
	}
}
```

```bash curl
# Direct call: pass the graph payload yourself.
curl -X 'POST' \
  'https://api.retab.com/v1/workflows/wf_abc123/diagnose-graph' \
  -H 'Content-Type: application/json' \
  -H 'Api-Key: <your-api-key>' \
  -d '{
    "blocks": [
      {
        "id": "start-1",
        "type": "start-document",
        "label": "Start",
        "config": null,
        "position": { "x": 0, "y": 0 }
      }
    ],
    "edges": [],
    "re_propagate": true
  }'
```

</RequestExample>

<ResponseExample>

```json 200 (valid)
{
  "is_valid": true,
  "issues": [],
  "suggestions": [],
  "stats": {
    "total_blocks": 5,
    "total_edges": 4,
    "block_types": { "start": 1, "extract": 2, "classifier": 1, "split": 1 },
    "start_document_blocks": 1
  }
}
```

```json 200 (valid with warnings)
{
  "is_valid": true,
  "issues": [
    {
      "severity": "warning",
      "code": "MISSING_HIL_PREDICATE",
      "message": "Extract block 'Extract' has human review enabled but no predicate configured.",
      "block_id": "extract-1"
    }
  ],
  "suggestions": [],
  "stats": {
    "total_blocks": 2,
    "total_edges": 1,
    "block_types": { "start": 1, "extract": 1 },
    "start_document_blocks": 1
  }
}
```

```json 200 (with issues)
{
  "is_valid": false,
  "issues": [
    {
      "severity": "error",
      "code": "NO_START_BLOCK",
      "message": "Workflow has no start-document block.",
      "block_id": null
    },
    {
      "severity": "warning",
      "code": "UNREACHABLE_BLOCK",
      "message": "Block 'extract-2' is unreachable from any start-document block.",
      "block_id": "extract-2"
    }
  ],
  "suggestions": [
    "Add a `start-document` block, or remove the orphaned `extract-2` block."
  ],
  "stats": {
    "total_blocks": 4,
    "total_edges": 2,
    "block_types": { "extract": 2, "classifier": 1, "split": 1 },
    "start_document_blocks": 0
  }
}
```

</ResponseExample>
