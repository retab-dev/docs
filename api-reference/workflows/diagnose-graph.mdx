---
title: 'Diagnose Workflow Graph'
openapi: 'POST /v1/workflows/{workflow_id}/diagnose-graph'
---

Validate an in-memory workflow graph and return any structural issues.
This is the authoritative validation surface for the workflow editor —
both for an unsaved draft (passing `blocks` and `edges` directly) and for
the persisted draft (fetching them first via
[Get Workflow Entities](/api-reference/workflows/get-entities) and
forwarding them to this endpoint).

The check covers:

- Missing or duplicate start blocks
- Disconnected blocks
- Dangling edges (one endpoint missing)
- Unreachable blocks
- Type mismatches between source/target handles
- Missing required block configuration
- Cycles in the directed graph

Severities:

| Severity | Meaning |
|---|---|
| `error` | Must fix before publish — block runs would fail. |
| `warning` | Should fix; runs may still succeed. |
| `info` | Advisory only. |

`is_valid` is `true` when the issue list contains no errors. `re_propagate`
defaults to `true` so derived schemas are recomputed before validation —
turn it off if you've already propagated and want to validate the cached
shape.

The Python SDK exposes a one-shot helper that fetches the persisted draft
and POSTs it for you:

<RequestExample>

```python python
from retab import Retab

client = Retab()

# One-shot — fetches the persisted draft entities and diagnoses them.
diagnosis = client.workflows.diagnose("wf_abc123")
if not diagnosis.is_valid:
    for issue in diagnosis.issues:
        print(f"[{issue.severity}] {issue.code}: {issue.message}")
```

```javascript javascript
import { Retab } from '@retab/node';

const client = new Retab();

// One-shot — fetches the persisted draft entities and diagnoses them.
const diagnosis = await client.workflows.diagnose("wf_abc123");
if (!diagnosis.is_valid) {
  for (const issue of diagnosis.issues) {
    console.log(`[${issue.severity}] ${issue.code}: ${issue.message}`);
  }
}
```

```go go
package main

import (
	"context"
	"fmt"
	"log"

	retab "github.com/retab-dev/retab/clients/go"
)

func main() {
	ctx := context.Background()

	client, err := retab.NewClient("")
	if err != nil {
		log.Fatal(err)
	}

	// One-shot — fetches the persisted draft entities and diagnoses them.
	diagnosis, err := client.Workflows.Diagnose(ctx, "wf_abc123")
	if err != nil {
		log.Fatal(err)
	}

	if !diagnosis.IsValid {
		for _, issue := range diagnosis.Issues {
			fmt.Printf("[%s] %s: %s\n", issue.Severity, issue.Code, issue.Message)
		}
	}
}
```

```bash curl
# Direct call: pass the in-memory graph payload yourself.
curl -X 'POST' \
  'https://api.retab.com/v1/workflows/wf_abc123/diagnose-graph' \
  -H 'Content-Type: application/json' \
  -H 'Api-Key: <your-api-key>' \
  -d '{
    "blocks": [
      {
        "id": "start-1",
        "type": "start",
        "label": "Start",
        "config": null,
        "position": { "x": 0, "y": 0 }
      }
    ],
    "edges": [],
    "re_propagate": true
  }'
```

</RequestExample>

<ResponseExample>

```json 200 (valid)
{
  "is_valid": true,
  "issues": [],
  "suggestions": [],
  "stats": {
    "total_blocks": 5,
    "total_edges": 4,
    "block_types": { "start": 1, "extract": 2, "classifier": 1, "split": 1 },
    "start_blocks": 1
  }
}
```

```json 200 (with issues)
{
  "is_valid": false,
  "issues": [
    {
      "severity": "error",
      "code": "NO_START_BLOCK",
      "message": "Workflow has no start block.",
      "block_id": null
    },
    {
      "severity": "warning",
      "code": "UNREACHABLE_BLOCK",
      "message": "Block 'extract-2' is unreachable from any start block.",
      "block_id": "extract-2"
    }
  ],
  "suggestions": [
    "Add a `start` block, or remove the orphaned `extract-2` block."
  ],
  "stats": {
    "total_blocks": 4,
    "total_edges": 2,
    "block_types": { "extract": 2, "classifier": 1, "split": 1 },
    "start_blocks": 0
  }
}
```

</ResponseExample>
