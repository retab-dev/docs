...
<ParamField body="endpoint" type="string" required>
  ...
  **Resource-oriented (recommended):**
  ...
  <Note>
  Legacy document-scoped endpoints are still accepted for backward compatibility, but they are deprecated. New jobs should target the resource-oriented URLs so the result is retrievable via `GET /v1/{resource}/{id}`.
  </Note>
</ParamField>
