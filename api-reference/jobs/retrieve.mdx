...
// Retrieve a job by ID
const job = await client.jobs.retrieveFull("job_V1StGXR8_Z5jdHi6B-myT");
...
func main() {
	...
	// Retrieve a job by ID
	job, err := client.Jobs.RetrieveFull(ctx, "job_V1StGXR8_Z5jdHi6B-myT")
	if err != nil {
		log.Fatal(err)
	}
...
<Note>
For tight polling loops, use [`GET /v1/jobs/{job_id}/status`](/api-reference/jobs/status). It avoids request/response restoration work and returns only lifecycle metadata.
</Note>
...
// Fetch full payload only once the job is terminal
if (job.status === "completed") {
    job = await client.jobs.retrieveFull(jobId);
}
