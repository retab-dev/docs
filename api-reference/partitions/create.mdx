...
Partition a document into repeated chunks keyed by a value such as `invoice_number`, `policy_id`, or `claim_number`. Unlike splits, this endpoint is key-based grouping only and returns a canonical `output/consensus/usage` response without creating a persisted resource.
